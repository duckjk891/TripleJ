...
## v3.87 — 2026-08-27 — 내 목소리 만들기 문구·배치 정리 (위저드 UX)

### 요청(원문 요지)
① "정식 클로닝" 대신 "내 목소리 만들기"로 표기 ② "(노래+문장낭독)" 문구 부담스러우니 제거 ③ 진입 시 "최소 15초~2분" 안내 ④ "샘플 방식(노래로 부름/말로 함)"이 뭔지 헷갈림 — 방금 넣은 샘플의 종류 선택이므로 샘플 입력 바로 아래로 이동+명확한 문구.

### 수행
VoiceManage 버튼·설명("내가 부른 노래로 나만의 목소리를 만들어요"), 위저드 헤더 "내 목소리 만들기", 힌트 "최소 15초 ~ 2분"(기존 10초 오기 정정), 샘플 종류 선택을 샘플 입력 직하로 이동 + "방금 넣은 샘플은 어떤 음성인가요?" + [노래][말] 칩, 버튼 "다음: 만들기 시작". 3단계 낭독 안내 문구도 라벨 동기화. 사용자 노출 문자열에서 클로닝/문장낭독 제거(주석·로그 제외).

---

## v3.86 — 2026-08-27 — 이모지 제거: 작업실·디렉터 작업(대화)·마이페이지·나의 아티스트 (⭐ 재화만 예외)

### 요청(원문)
"작업실, 그리고 디렉터와의 작업(대화), 마이페이지, 나의 아티스트에서 스타(재화)를 제외하고 사용되는 모든 이모지 다 제거해." + 보충: "이모지는 아이콘이 아니야. 말그대로 이모지" — 벡터 아이콘 컴포넌트는 유지, **문자 이모지만 제거**.

### 범위 화면
작업실(Map·WaitTimer·DirectorLineup), 디렉터 작업/대화 전체(Dialogue, Artist Input/Cody/Loading/Result, Lyrics Input/Loading/PromptReview/Result/Book, Composer Input/Select, MusicGeneration/Loading/Result, CoverGeneration, VoiceManage, VoiceCloneWizard, MyArtists) + dialogues/ 대사 데이터, 마이페이지(해당 라우트 실확인). 차트/피드/검색/플레이리스트 등 다른 탭은 **범위 밖**.

### 규칙
- ⭐(U+2B50, 재화 표기)만 예외 — 나머지 문자 이모지 전부 제거.
- "📷 사진 올리기" 식 라벨 → 이모지만 떼고 텍스트 유지(공백 정리).
- 이모지 단독 버튼(예: 🗑) → 기존 앱 관행의 벡터 아이콘(@expo/vector-icons) 또는 짧은 텍스트로 대체(기능·터치 영역 불변).
- 로그·주석 내 이모지는 무관(사용자 노출 텍스트만).

### 리스크·회귀
버튼 의미 소실 방지(단독 이모지 버튼 대체 필수) · 다이얼로그/채팅 대사 문자열 내 이모지 포함 여부 전수 스캔(유니코드 범위 grep) · ⭐ 오삭제 금지 · 범위 밖 화면 오염 금지.

---

## v3.115~116 — 2026-08-31 — 대표 피드백: 마이페이지 구조 개편 + 코디 자유 디렉팅 + 아티스트 없음 버그

- v3.115(진행): 마이페이지 상위 탭 곡·앨범/피드/커뮤니티(곡·앨범 안에 곡/앨범/작사 서브탭), 피드 [새 피드 작성]·커뮤니티 [새 공지 작성](FeedCompose kind=community — 텍스트 전용 계약), 앨범 탭=새 앨범 만들기만(커버 보관함 버튼 제거 — 화면 보존)+앨범 탭→관리 이동 확인, 프로필에 앨범·팔로워/팔로잉 수 추가·레벨 제거.
- v3.116(진행): ①[버그] 아티스트 저장 후 "없음" 표시 — 실데이터(Mongo)·로그·코드로 원인 확정 후 수정(레거시 폴백 조건 의심) ②코디 상세설정 자유 디렉팅 입력(프롬프트 합성).
- 확인 답변: 커버 보관함=본인 산출물만 조회(서버 보장) — 대표 질문 회신.

---

## v3.113~114 — 2026-08-31 — 대표 피드백: 저장 버튼 + 마이페이지 피드·커뮤니티

- v3.113(완료): 아티스트 생성 완료 [저장하기] — 실제 멱등 재저장·"저장 완료" 표시(자동 저장 무음 실패 안전망 겸).
- v3.114(진행): 마이페이지=MAIDOL 내 채널 정합 — 피드·커뮤니티 탭 추가(내 글 목록, FeedCard 재사용), 작곡 탭 "음원 파일 올리기" 진입 제거(기능·화면은 보존, 레퍼런스 업로드는 작곡 흐름의 참고음악으로 이미 존재).

---

## v3.111~112 — 2026-08-31 — 대표 피드백: 피드 이미지 첨부 + 아티스트 추가 시 실사/가상 선택

### 범위
- v3.111(진행): 피드 작성 이미지 첨부(최대 4장) — 백엔드(우리 소유): 이미지 블록·업로드·**서버 재인코딩(긴 변 1600px 초과 시만 LANCZOS 축소·q85·EXIF 정리 — 용량 관리하되 퀄리티 보존, 원본이 작으면 무손실 유지)** / 프론트: 첨부·미리보기·FeedCard 렌더. 피드 영속은 기존 확인(서버 DB 저장 중).
- v3.112(대기 — v3.109와 파일 충돌 회피): 아티스트 추가 시 실사/가상 선택 둘 다 노출(현재 실사 경로 미노출) — v3.109 완료 후 착수.

---

## v3.109~110 — 2026-08-31 — 대표 피드백: 아티스트 흐름 5건 + 작사 프롬프트 개편

### 범위
- v3.109(진행): ①추가 흐름에서 현재 아티스트 꾸미기 숨김 ②디렉터 대화 '우리 {아티스트}'의 '우리' 제거 ③"이 화풍으로 만들기" 버튼 세로 잘림 재수정(버튼 자체 높이 — v3.105는 여백만 제거했었음) ④아티스트 이름 질문 추가(save name 영속) ⑤코디 옷 아이템 링크 노출(link_url)
- v3.110(진행): 작사 프롬프트 체계 개편 — 대화 수집은 유지, 사용자 화면=항목별 쉬운 요약 카드(어색한 자연어 문단 제거), API 전달=백엔드 lyrics_generator 시스템 프롬프트에 최적인 구조로 정합. 설계 문서 작사_프롬프트_개편_v1.md 산출.

---

## v3.108 — 2026-08-31 — 백엔드 직접 인수 1차: B-9 ⭐5·B-10 서버 폴링·B-11 album_id + 프론트 v217 대응

### 배경
- 대표: 클로닝 과금 **⭐5 확정**("작곡에 포함된 수준" — 팀 동의), 백엔드 직접 소유 선언(같은 레포 backend 브랜치 → 로컬 워크트리 TripleJ-backend, v217 인수).
- 실측: 9004 프로세스(18:53)가 v217 커밋(18:57)보다 오래됨 — v217 미적용 상태, 이번 배포 시 재시작으로 함께 적용.

### 범위
- 백엔드(직접): B-9 voice_clone ⭐5(차감+실패 환불+costs), B-10 진행 클론 30초 주기 폴링+30분 타임아웃, B-11 _serialize_track album_id(배치 역조회)
- 프론트: v217 refine character_id 전송(화풍 붕괴 방지)·image_model 정리, 클로닝 비용 조건부 고지(costs.voice_clone 존재 시만)
- 배포: rsync→9004 재시작→[api] 검증

---

## v3.105~107 — 2026-08-31 — 대표 피드백 8건 대응

### 실측 선행 결과
- **클로닝 생성 멈춤(#4)**: 서버는 GET /voice-clone/{id} 시에만 Suno 폴링(auto-poll on GET) — 앱이 "백그라운드" 안내 후 폴링 중단 → 2.5h 방치 → Suno 문구 만료로 실패 확정(대표 클론 "진주" failed 처리, 재진행 필요). 프론트=생성 중 폴링 유지(v3.105), 백엔드=서버측 주기 폴링 요청(B-10).
- **비용표 실측**: lyrics 5 / compose 15 / cover 5 / character 10 / fatigue_skip 5 / hire_director 10 / extra_slot 15.

### 배분
- v3.105(진행): 클로닝 폴링 유지·아티스트 추가 ⭐10 선행 팝업+데이터 보존·화풍 버튼 잘림/작업실 하단 공백 제거·setState 루프 픽스
- v3.106(진행): 최신앨범 섹션 신곡 탭 이동·앨범 소속 곡 클릭→앨범 페이지(트랙 album_id 계약 실측 후)
- v3.107(대기): 디렉터 진행 UI 개편 — 광고 대기 큐 폐지→즉시 결과+피로도 게이트 체계 반영 (v3.105 완료 후, MapScreen 충돌 회피)
- 리서치(진행): 작사 구조화 블럭 자료 조사
- 과금 제안(#3): 클로닝 ⭐20(create 차감·실패 자동 환불) — B-9로 백엔드 요청

---

## v3.104 — 2026-08-31 — B-5 연동: 커버 보관함 UI

### 범위
- GET /upload/cover-sessions 목록 화면(마이페이지 근처) — 커버 카드(사용 중 곡 표시), 삭제(미사용만 — 409 linked_tracks 안내), 재사용(곡 커버 변경·발매 시 선택).
- 백엔드 확인 후보 회신 정리(잡 결과 character_id).
...
### 범위 (감사 A-19)
- /auth/guardian-consent/* 실측(실서버 openapi 우선) → 가입 플로우에 만 14세 미만 분기 + 보호자 동의 절차. "본인인증/보호자 인증 시 ⭐ 지급" 방침 연계(본인인증 PG는 심사 후 — 지급 트리거는 서버 소관 확인).

---

## v3.100 — 2026-08-31 — 직접 음원 파일 업로드 (대표 정책 확정: 허용)

### 범위 (감사 A-10)
- POST /tracks/upload(+이미지) — 내 음원 파일 직접 발매. 저작권은 신고 기능 사후 검열 방침(대표 확정).
- 이후 v3.101 보호자 동의(A-19, 확정), MV(A-1)는 MVP 미반영 확정.

---

## v3.98 — 2026-08-29 — 파리티 Wave 8: Kits 음성 변환 + MR 피치 (서버 다운 중 코드 선행)

### 범위 (감사 순서 6 후반, 자율 진행 마지막 Wave)
- **Kits 음성 변환 + MR 피치 프리뷰/병합(A-8)**: /voice-convert/*, /kits/voice-models — MAIDOL 실측 후 이식. 비용(⭐) 여부 실측, 껍데기 과금 금지.
- 이후 잔여: 직접 업로드(A-10)·보호자 동의(A-19)=정책 판단, MV(A-1)=에픽, 피드 작성기 다중블록(B-1)·Wondera 보컬(A-7) — 사용자 보고 후 결정.

---

## v3.97 — 2026-08-29 — 파리티 Wave 7: 비트뷰/메트로놈 + 차트 일간 탭 (서버 다운 중 코드 선행)

### 범위 (감사 순서 6 전반 + B-갭 소형)
- **비트뷰/메트로놈(A-9)**: /tracks/{id}/beats — 비트 시각화·재시도. MAIDOL 실측 후 플레이어에 이식.
- **차트 일간 탭(B-3)**: 차트에 "일간" 탭 추가(서버 파라미터 실측).
- Kits 음성변환(A-8)은 v3.98로 분리. 직접 업로드(A-10)·보호자 동의(A-19)·MV(A-1)는 정책/에픽 — 사용자 판단 대기로 보류.

---

## v3.96 — 2026-08-29 — 파리티 Wave 6: 앨범 관리 + 홈 최신앨범 (서버 다운 중 코드 선행)

### 범위 (감사 순서 5)
- **앨범 관리(A-2)**: /albums/* — 내 앨범 생성·수정·삭제, 트랙 추가/제거/정렬, 커버 업로드·AI 커버. 현재 AIDOL은 열람만.
- **홈 최신앨범(A-20)**: GET /albums/latest 섹션.

계약 로컬 실측·tsc만 검증·PENDING_TESTS 등록.

---

## v3.95 — 2026-08-29 — 파리티 Wave 5: 신뢰·CS 묶음 (서버 다운 중 코드 선행)

### 범위 (감사 순서 4)
1. **소명(Appeal) 플로우**(A-13): /reports/my-affected + /{id}/appeal — 블라인드된 내 콘텐츠 목록·소명 제출
2. **CS 오류신고 → 공식계정 DM**(A-14): GET /dm/official → 기존 DM 화면으로 연결
3. **피드 단건 딥링크 착지**(A-21): GET /feeds/{id} — 피드 상세 화면 + 앱 링크 설정(가능 범위: expo linking 구성, 네이티브 빌드 필요 부분은 문서화만)

계약 로컬 실측·tsc만 검증·PENDING_TESTS 등록.

---

## v3.94 — 2026-08-29 — 파리티 Wave 4: 디렉터 피로/쿨다운 (서버 다운 중 코드 선행)

### 범위 (감사 순서 3 후반)
- **피로/쿨다운(A-3)**: /fatigue/* — 연속 생성 시 2/4/8/12h 쿨다운 사다리, ⭐5·광고 스킵, 생성 429 분기. MAIDOL 실측 후 AIDOL 생성 흐름에 이식.
- **Wondera 보컬 업로드(A-7)는 보류**: AIDOL 쪽 Wondera 경로가 dead path인지 실측 후 판단 — 이번 Wave에서 확인만 하고 구현은 감사 6단계로 이월.

계약 로컬 실측·tsc만 검증·PENDING_TESTS 등록. 광고 스킵은 실제 AdMob 미연동 시 MAIDOL 관행 확인 후 ⭐ 스킵만 우선.

---

## v3.93 — 2026-08-29 — 파리티 Wave 3: 작업실 완성 1 — 생성 이력 + 2-variant 비교 (서버 다운 중 코드 선행)

### 범위 (감사 순서 3 전반)
1. **생성 이력 목록·삭제·이어서 작업**(A-4 전반): GET /generate/ 목록, DELETE /generate/{id}, 미완료 건 이어서 작업(폴링 재개/결과 화면 복귀)
2. **2-variant 클립 A/B 비교**(A-4 후반): Suno가 주는 2개 변형 클립을 모두 청취·선택 (stream?variant=)
- 피로/쿨다운(A-3)·Wondera 보컬(A-7)은 v3.94로 분리.

계약은 로컬 MAIDOL+backend_9004 실측. tsc만 검증, 실행 테스트 PENDING_TESTS 등록. 순차 진행(음악 생성 화면들 v3.91과 파일 겹침 → 병렬 금지).

---

## v3.92 — 2026-08-29 — 파리티 Wave 2: 계정 위생 묶음 (서버 다운 중 코드 선행)

### 범위 (감사 순서 2)
1. **프로필 이미지 업로드/삭제**(A-16): /auth/me/profile-image — 현재 표시만 있고 업로드 없음
2. **마케팅 동의 토글·이력**(A-17): /auth/me/consents
3. **인구통계 프로필 편집**(A-18): PATCH /auth/me/profile — 생년월일/성별/지역(+SNS 채널, MAIDOL 범위 따름). 현재 회사명/직함/bio만 편집 가능
- 회원탈퇴(A-15)는 v3.91 선반영 완료. 보호자 동의(A-19)는 중형·정책 판단으로 7단계 보류.

계약은 로컬 MAIDOL+backend_9004 스냅샷으로 확정, tsc만 검증, 실행 테스트는 PENDING_TESTS 등록. v3.91과 같은 SettingsScreen을 다시 만지므로 순차 진행(병렬 금지).
...
- MAIDOL ItemSelectModal.jsx 계약: 드릴다운 = 플랫폼→브랜드→성별→제품→색상 5단계, API 아닌 **클라이언트 패싯 필터**(getActiveAds 결과). 위시리스트 서버 계약(wishlist.py): toggle/check/list.
- AIDOL ArtistCodyScreen 모달 내부 확장(구조 대체 아님): [전체(드릴다운) | 내 위시리스트] 탭, 카드 하트 토글(Feather — 이모지 금지 방침 준수), 위시 탭에서 바로 코디 적용, 판매종료 배지. 기존 선택·프롬프트·object_name 경로 무변경. wishlistStore 신설(낙관적 토글+롤백, check 일괄 sync, 카테고리는 클라이언트 필터).
...
### 요청(원문 요지)
① 목록의 실사/가상 표시 전부 제거 ② 추가 시 "가상(그림)으로 만들어져요" 문구 금지 — 추가는 kind 무관 "슬롯 하나 추가" ③ 상세 클릭 시 아티스트 이름·성별 표시(실사/가상 표기 제거) ④ 다시만들기와 삭제가 같으면 삭제 버튼 제거 ⑤ 꾸미기 버튼은 탭바 바로 위 — 미니플레이어 재생 중이면 이 화면에선 백그라운드 재생(숨김), 나가면 다시 표시.

### Plan verification findings
- ④ 실확인: "🗑 캐릭터 다시 만들기"(스크롤 끝)와 [🗑 삭제](v3.79 하단 버튼) 모두 동일한 `DELETE /character/me` 전체 삭제 — 차이는 삭제 후 목적지뿐(재생성 진입 vs 목록). **동일 기능 → 하단 [🗑 삭제] 버튼 제거**(사용자 지시), 다시 만들기 박스만 유지.
- ② 서버 제약: 현 서버는 빈 슬롯 kind로만 생성 가능(같은 kind 재생성=기존 덮어씀) → **내부 forceKind는 유지하되 UI 문구·표기에서 kind 완전 제거**. B-1 반영 시 자유화.
- ③ 서버 character에 name은 있으나 gender 필드 유무는 /me 실측 필요 — 없으면 생성 질문 답변(성별)을 로컬 persist로 슬롯별 저장해 표시, 백엔드 요청서에 gender 필드 추가 항목 기재.
- ⑤ 현재 bottomArea가 `bottomLift(MINIPLAYER_HEIGHT)`로 미니플레이어 위로 올라감 — ArtistResult(필요 시 MyArtists 포함)에서 미니플레이어 숨김(오디오는 계속) + bottomLift 제거.

### 범위 (전부 프론트)
1. MyArtists: kind 배지·화풍 라벨 제거(카드=썸네일+이름), 추가 confirm 문구 "⭐{cost}를 사용해 아티스트 슬롯을 추가할까요?"로만.
2. ArtistResult: kind 배지 제거 → 이름·성별 표시(서버 name, gender는 실측 후 서버 값 또는 로컬 저장분). 하단 [🗑 삭제] 제거, [✨ 꾸미기]만(가상 슬롯은 기존대로 숨김 조건 유지하되 표기는 없음).
3. 미니플레이어: ArtistResult 포커스 중 숨김(재생 유지), blur 시 복원. bottomArea는 탭바 바로 위로.
4. ArtistInput: forceKind 시 안내 채팅에서도 kind 문구 제거(흐름은 그대로).
5. BACKEND_WORK_REQUEST: character 스키마에 gender 필드 추가 요청 기재.

### 리스크·회귀
미니플레이어 숨김이 다른 화면 복원에 영향 없어야(포커스 기반 정리 필수) · 삭제 경로는 다시만들기 박스로 일원화(삭제 후 목록 재로드 유지) · kind 제거해도 내부 로직(cartoon 분기·화풍 스텝)은 불변.
...
## v3.129 — 장르/사운드 질문 통합 (대표 재결정, 2026-09-07)
...
## v3.136 + 백엔드 v230 — 가사 저장소 단일화 (2026-09-08)
...
## v3.184 — 2026-09-16 — 워터마크 정리(심볼 제거·텍스트형·항상 최하단)
**요청**: 워터마크가 커버 이미지(예전 텍스트형)와 다름. 자막과 별개로 하단에. 깔끔하게 — 별모양 응원봉 로고 제거.
### findings/조치
