...
# v227 보강 (2026-09-01) — 도안 개수·배치 고정 + 하의 템플릿 개편

- 템플릿 하의 칸 `Skirt:` → `Bottom (pants / shorts / skirt):` (Waist에 elastic/belted, Pleats→Pleats/Tucks), Logo/Print에 "COUNT and exact placement — never turn a single graphic into a repeating pattern" 명시.
- [도안 개수·배치 규칙] 신설(만화 화풍변환규칙·Final Constraint·Step B 3종·step1 하의): 참조에서 한 곳에만 있는 단일 도안은 결과에서도 그 위치에 그 개수만 — 복사·반복 패턴화 절대 금지, 전면 패턴은 동일 밀도로. 상의·하의·신발 공통 적용.
- FE(v3.126): 하의 특별 지시가 참조 이미지 존재 시 "참조 사진의 실제 제품 그대로(종류·색·핏·기장·허리 형태·프린트 개수·배치)"로 교체, 최종 점검에 프린트 개수·위치 확인 추가.

---

# v228 보강 (2026-09-02) — MAIDOL 이식 2건

- **REALISTIC OVERRIDE (실사+사진 상시)**: AI 기본 미화 거부(대칭 보정·피부 리터칭·V라인/8등신화 금지), 비대칭·피부결·일반인 비율 사진 그대로, [Final Constraint]="더 예쁜/잘생긴 결과=실패". §4만 AIDOL 조정: 촬영 퀄리티(조명·구도)는 프로필급 허용, 인물 자체 미화만 금지. STEP 1에도 realistic suffix 부가.
- **Step A 모델**: Claude(claude-opus-4-7) 우선 → 실패/키 미설정 시 Gemini(gemini-3.1-pro-preview, v224 재시도) 폴백. 실사·만화 공통. 역할 라벨([상의 참조] 등) 텍스트 파트를 Claude content에 그대로 보존.
