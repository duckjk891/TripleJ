...
1. [unit] tsc / 구명칭 하드코딩 잔존 0 (STYLE_EN 하위 호환 항목 제외)
2. [e2e] 작사 대화 진행(장르→분위기) → 사운드 질문에 역할 구별 문구 + 신명칭 칩 노출 + 구명칭 미노출
3. [회귀] STYLE_EN 구명칭 매핑 보존 — 기존 저장 가사로 작곡 시 영문 태그 정상

---

## v3.129 (2026-09-07)

1. [unit] tsc / 하드코딩 스텝 인덱스 잔재 검사(듀엣2·랩8·길이9 동기화 확인)
2. [e2e] 대화 전체 진행 — 분위기 다음=듀엣(사운드 질문 없음) → 11단계 완주 → 요약 도착·장르 표시·스타일 카드 없음
3. [회귀] GENRE_DEFAULT_STYLE 파생 — style 빈값+장르 존재 시 en.style 자동 주입 (콘솔 로그 확인용 [Suno] style 자동 파생)
...
1. [unit] tsc
2. [e2e] 작곡 디렉터 진입 → 대화 후 "어떤 가사로 작곡할까요?" 선택 화면 도착 → 서버 가사 최신순(신규 시드가 기존 항목보다 위) → 선택·작곡하기 → 작곡 진행 착지
3. [회귀] 일반 보관함 진입(작사 디렉터 경로)은 기존 문구·기능 유지 / 드래프트 삭제 시도 → 안내 팝업
...
1. [unit] tsc
2. [e2e] 작곡 디렉터 → 대화 → 대화형 선택 화면(디렉터 말풍선·출처 배지·최신순) → 선택 → 작곡 진행 착지
3. [api-간접] 자동 자산 저장 — 다음 실작사 시 save:true 전송(lyrics_id 응답) 확인은 차기 작사 회귀에서
4. [회귀] 발매곡 가사 포함·중복 제거(보관함=발매곡 동일 내용 시 1건)
...
1. [unit] tsc
2. [e2e] [가사 보기] → 프리뷰에 잘리던 라인 노출 + [접기]·[이 가사로 작곡하기] 표시 → 접기 복원 → 펼친 상태 작곡 버튼으로 작곡 진행 진입
3. [회귀] 카드 탭 바로 작곡 유지
...
1. [unit] tsc
2. [e2e] 자산 가사 선택 → 작곡 13단계에서 제목 "(수정판)"·가사에 검증 라인 추가 → [음악 생성 시작] → PATCH /lyrics/{id} 발신 감지 (작곡 POST는 route abort로 무과금) → 서버 자산 재조회 시 제목·가사 갱신 확인
3. [회귀] 제목·가사 수정이 생성 파라미터(store)에 반영(기존 동작) / PATCH 실패해도 작곡 진행

---

## v3.135 (2026-09-08)

1. [unit] tsc / character_id 필드 확인
2. [e2e] 가사 선택(장르 록·분위기 없음) → 2단계 안내 "장르: 록, 분위기: 자동"(빈칸 없음) → 아티스트 질문·카드·건너뛰기 노출 → 목소리 없는 아티스트 선택 → "연결된 목소리가 없어요" + 보컬 질문 진입
3. [보류] 목소리 연결 아티스트 자동 반영 실검증 — persona 연결 계정 필요(클로닝 자산). 코드 경로: personaId 설정+step5 직행+step12 자동 통과. 대표 실사용 확인 항목
4. [회귀] 아티스트 0명 계정은 기존 흐름(보컬 직행)

---

## v3.136/v230 (2026-09-08)

1. [api] 이관 실측 — 대표 계정 자산 9건(발매곡 이관분 포함, note 표기·원 생성일 보존)
2. [e2e] 선택 화면 — 서버 자산만 표시(/tracks/my 미호출 확인), 보관함 배지, draft 고정 유지
3. [unit] tsc·py_compile
4. [회귀] 발매 훅 — 다음 실발매 시 auto lyrics-asset 로그([SongSource] auto lyrics-asset) 확인 예정

---

## v3.137 (2026-09-08)

1. [api] 가사 DB 실값 실측(대표 9건 genre/mood 전부 존재)
2. [e2e] mood 없는 가사 선택 → '자동' 미표기 + 분위기 질문(301) → 선택 → 아티스트 스텝에 시트 썸네일(preview img) → 성별 '여성' 아티스트 선택 → "아티스트를 따라 여성" + 보컬 스타일 선택지 직행
3. [회귀] 값 완비 가사는 실값 안내 후 기존 흐름 / gender 미매핑 아티스트는 기존 성별 질문

---

## v3.138 (2026-09-08)

1. [api] DB 무결 검증 — 9건 genre/mood 보유·updated_at 무변
2. [e2e] 대표 케이스 재현 — 장르 빈 드래프트(내용=DB 자산과 동일) 주입 → 선택 → "장르 정보 없음" 질문 미출현·장르 '록' 승계, 분위기만(그 자산에 실제로 없음) 질문
3. [회귀] 자산 직접 선택·장르/분위기 완비 케이스는 실값 안내 유지

---

## v3.139 (2026-09-08)

1. [unit] tsc / POINT_COSTS voice_clone=5 확인·persona 사용 무과금 확인
2. [e2e] 아티스트 없이 진행 → 성별 단계에 내 목소리 옵션 → 클론 없음 계정: 만들기(⭐5)/돌아가기 노출 → 돌아가기 시 성별 질문 복귀
3. [보류] 클론 보유 계정의 선택→persona 적용→스킵 경로 — 코드 검증(아티스트 목소리와 동일 로직 재사용), 대표 실사용 확인

---

## v3.140 (2026-09-08)

1. [api] voice_clones 계정 스코프·녹음 파일 필드(source/verify object) 실측 — 대표 클론으로 확인
2. [e2e] 마이페이지 "내 목소리" 행 노출 → 탭 → 목소리 관리 화면 진입
3. [회귀] 내 아티스트 행·기존 진입로(아티스트 상세/작곡 중) 무영향

---

## v3.141 (2026-09-09)

1. [api] 서버 프로세스 PATH에 .local/bin 포함 + normalize_audio_bytes 20초 wav 실검증
2. [e2e] 마이페이지→내 목소리: '현재 아티스트 목소리'·'간편 만들기'·2택 문구 미노출, [내 목소리 만들기]+[목록]만
3. [회귀] 아티스트 상세→목소리 진입은 기존 화면(간편/연결 포함) 유지 / 위저드 헤더 '내 목소리 만들기' 확인
4. [실사용] 대표 샘플 재업로드 → 422 미발생 (ffmpeg 복구 후)

---

## v3.143 — 아티스트 목소리 2택·필수화 + 작곡 목소리 질문 재편 (2026-09-09)

- [api] A1: PATCH /character/{cid} voice_preset="female:소프트" → 200, 응답 voice_preset 반영, persona_* 해제 상태 유지
- [api] A2: PATCH voice_preset="x:소프트"(무효 성별) → 400
- [api] A3: PATCH voice_preset="" → 해제(빈 문자열)
- [api] A4: persona_id(clone)와 voice_preset 동시 설정 전송 → 400
- [api] A5: GET /character/list 응답에 voice_preset 필드 동봉
- [api] A6(회귀): 기존 persona_id 연결/해제 PATCH 정상 동작
- [e2e] E1: 작곡(무아티스트) — 가사 선택→건너뛰기→성별 질문(내 목소리 버튼 없음)→목소리 질문(간편/내 목소리)→간편→스타일→진행, step 12 자동 통과
- [e2e] E2: 작곡 — 목소리 미연결 아티스트 탭 → 차단 다이얼로그(연결 안내), 진행 안 됨
- [e2e] E3: 작곡 — 간편 목소리 연결 아티스트 선택 → 성별·스타일 자동 반영, 보컬 질문 없이 step 5 직행
- [e2e] E4: 아티스트 상세 — 목소리 연결(필수) 버튼 → 2택(간편/내 목소리) → 간편 성별·스타일 연결 → 상태 문구 갱신
- [e2e] E5(회귀): 내 목소리(클론) 단계 210 돌아가기 → 목소리 질문(220) 복귀

---

## v3.144 — 작업본 출처 고리 영속화 (2026-09-09)

- [e2e] D1a: 어긋난 작업본(내용 상이·장르 없음·출처 없음·제목=자산 제목) 주입 → 카드에 장르·분위기 표시(제목 폴백 병합)
- [e2e] D1b: 병합된 자산의 이중 표시 제거(같은 제목 1회만)
- [e2e] D1c: 그 작업본 선택→가사 확인 → '장르 정보가 없네요' 질문 없음, "장르: 실값" 안내
- [unit] tsc 통과 / [회귀] 정상 자산 선택 흐름은 기존 병합(내용 일치) 경로 유지

---

## v3.145 — 고아 자산화 + 장르 확인 질문 (2026-09-09)

- [e2e] F1: DB에 없는 고아 작업본 주입 → 진입 시 자동 자산화 로그 → 선택 → 장르 질문(값 없음 정상) → 장르·분위기 선택 → 진행. 종료 후 테스트 자산 삭제.
- [e2e] F2: 장르/분위기 있는 자산 선택 → "이 느낌 그대로 작곡할까요?" 질문 → 네 → "확인! (실값)" 후 아티스트 단계.
- [e2e] F3: 아니요 → 장르 질문 → 분위기 질문(둘 다 강제) → 재선택값으로 진행.
- [회귀] 장르/분위기 없는 가사는 기존 질문 흐름 유지(F1이 겸함). tsc 통과.

---

## v3.146 — 작곡 장르/분위기 직접 입력 (2026-09-09)

- [e2e] G1: 302 '아니요' → 장르 화면에 직접 입력행 → "신스팝" 입력·확인 → 분위기 화면 직접 입력행 → "쓸쓸한 새벽 감성" 입력·확인 → 입력값으로 진행(대화에 표시)
- [e2e] G2(회귀): 같은 화면에서 버튼 선택(발라드→잔잔하고 편안한)도 기존대로 동작
- [unit] 빈값 확인 버튼 비활성(스크립트에서 disabled 확인), tsc 통과

---

## v3.147 — 보이스 만료 대응 (2026-09-09)

- [api] H1: expired 클론 persona로 작곡(start_music_gen) → 400 만료 안내 + ⭐ 무차감
- [api] H2: 만료 클론 연결 아티스트 → /character/list persona_status='expired' (FE 카드/차단 문구 근거)
- [api] H3: refund_clone_points 원자성 — refunded=True 재호출 시 False(이중 환불 불가, 기구현 재사용)
- [운영] H4: 대표 만료 클론 ⭐5 환불 실집행 + 잔액 반영 확인
- [unit] tsc·py_compile 통과 / [회귀] 정상(ready·프리셋) 작곡 경로 무영향(체크 실패 시 통과 설계)

---

## v3.148 — V5_5 강제·TTL 워처·대화 되감기 (2026-09-09)

- [unit] I1: suno_generator resolved_model — persona 有+suno_model 無 → V5_5 / persona 無 → V5 / 명시값 우선 (코드 검증 + 배포본 grep)
- [운영] I2: TTL 워처 단일 인스턴스 가동·5분 간격 로그 적재, 만료 시 lifetime 기록 (실측은 만료 대기)
- [e2e] I3: 작곡 대화 — 성별 답변 말풍선 탭 → 확인 팝업 → '다시 선택' → 성별 질문 재출력 → 다른 성별 선택 → 목소리 질문 이어짐(이후 흐름 정상)
- [e2e] I4: 되감기 취소 — 팝업에서 취소 시 진행 상태 불변
- [e2e] I5(회귀): 302→간편→스타일 정상 진행(스탬프 추가로 기존 흐름 무영향), step12 자동 통과 유지
- [api] I6(회귀): 정상 compose 402/400 게이트 순서 불변 (만료 선체크 게이트 v3.147 재확인 겸)

---

## v3.150 — 이미지 디렉터 대화 개편 (2026-09-09)

- [api] J1: POST /upload/cover-background 이미지 201(+본인 prefix) / 비이미지 400
- [unit] J2: 프롬프트 조립(모킹) — 구도/색감/배경텍스트/배경사진 절/가사 발췌(무텍스트 지시)/장르·분위기 미주입/의상 유지 절/참조 2장
- [e2e] J3: 대화 전체 흐름 — 곡→아티스트 포함→의상 확인(미리보기)→구도(반신)→배경(말로 설명)→색감(다크 무디)→가사 건너뛰기→'이대로 만들기' → generate-cover 페이로드에 shot/palette/background_prompt 실림(요청 인터셉트)
- [e2e] J4: 의상 바꾸러 가기 → 아티스트 꾸미기 화면 진입 → 복귀 시 의상 확인 단계 유지
- [회귀] 아티스트 미포함 경로: '인물 없이' 구도 노출, 전 질문 건너뛰기만으로 생성 도달

---

## v3.151 — 커버 질문 재편·되감기 (2026-09-09)

- [e2e] K1: 곡→포함→의상 그대로→**가사 질문 등장**→"아니요, 직접"→구도→되감기(구도 답변 탭→다시 선택→재질문→다른 값)→배경 텍스트→색감→최종(예시 placeholder)→이대로 만들기 → 페이로드에 되감기 후 값 반영
- [e2e] K2: 가사 반영 선택 → 구도/배경/색감 질문 **미출현**, 최종 직행 → 페이로드 lyrics_excerpt 有·shot/palette 無
- [e2e] K3: 되감기 취소 시 상태 불변 / [회귀] tsc·J3 흐름 무결(K1이 겸함)

---

## v3.152 — 실사/가상 프롬프트 분기 (2026-09-09)

- [unit] P1: virtual+화풍 → "fully illustrated in {화풍}"·실사 금지·일러스트 연출·디자인/의상 유지·가사 절 공존 (모킹 7항목)
...
## v3.175 — 위시 하트 대비 + 위시리스트 화면
- L1 [e2e] 착장 카드 하트 담기 전 흰색(rgb 255,255,255) → 담은 뒤 핑크(rgb 255,77,109) 채워짐
- L2 [e2e] 마이페이지 → '내 위시리스트' 카드 → 화면에 담은 아이템·"담아둔 아이템 N개" 노출
- L3 [e2e] 위시리스트 화면에서 해제 하트 → 목록에서 제거(정리)
- [unit] tsc 통과

## v3.176 — 위시 하트 보라 + 위시 화면 제거
...
## v3.178 — 댓글 진입 개편 + 상세토글 미니플레이어 축소
- L1 [e2e] 플레이어 액션줄에 좋아요·댓글·담기·재생목록 노출(+큰 커버)
- L2 [e2e] 댓글 클릭 → 상세 열림·댓글 탭 활성·상단 미니바로 축소(큰 커버 숨김, 미니커버 노출), 화면 안 덮음
- L3 [e2e] 핸들('토글 내리기') → 큰 플레이어 복귀(큰 커버 재노출)
...
## v3.181 — 영상 스타일 확장 + 댓글 UI 정리
- A1 [api] 잘못된 font/fontcolor/bg 400
- A2 [api] serif+yellow+clean+center+circle+line 실생성 → 프레임 추출 시각 확인(원본 배경·원형·명조 노랑 한 줄)
- A3 [api] 동일 조합 재요청 cached:true, 과금 ref에 6축 포함(v3.179 조합 ref 하위호환)
- L1 [e2e] 디렉터 대화: 배경(흐림/원본)→폰트 3종→글자색 4종 견본→가사 방식 카드 전 단계 노출
- L2 [e2e] 댓글 계측: 이름 accent(168,85,247)+800, 아바타 32px, 폴백 배경=팔레트 색(랜덤), borderBottom/Left 0(라인 없음)
- 회귀: 피드 댓글 동일 패턴(공용 Avatar·연결선 제거) — tsc·코드 경로 동일

## v3.182
- A1 [api] 스타일 검증 400(bg=color+잘못된 hex 등), v6 실생성(kakao·center·circle·line·dohyeon·hex색·color bg·bold) 프레임 시각 확인(로고 워터마크 포함), 보관함 목록/object 프록시 200
- L1 [e2e] 댓글 입력창: 목록 아래(하단)·초기 높이 38 계측·미니바 ✕ 없음
- L2 [e2e] 아티스트 디렉터 → Dialogue(캐릭터+대화창) → 내 아티스트
...
## v3.190 — 2026-09-17 — 프로필 완성 보상 ⭐10

| # | 시나리오 | 태그 | 결과 |
|---|---|---|---|
| 1 | 일부만 채움(지역 null) 저장 → granted=false | [api] | PASS |
| 2 | 3종 완성 저장 → granted=true, 잔액 +10, [star-econ] 로그 | [api] | PASS |
| 3 | 재저장(값 변경) → 멱등 dup, granted=false | [api] | PASS |
| 4 | (회귀) 저장 응답에 기존 프로필 필드 유지·user 상태에 플래그 미혼입 | [unit] | PASS (tsc 0에러+응답 확인) |
| 5 | 신규 계정: 설정 진입 → "완성하고 ⭐10 받기" 배지 노출 | [e2e] | PASS (v3190_badge.png) |
| 6 | 지역 선택·저장 → "⭐10 지급 완료!" 팝업 → 배지 소멸 | [e2e] | PASS (v3190_granted.png) |
