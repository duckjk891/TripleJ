...
### v3.97 — 비트뷰/메트로놈 + 차트 일간 탭 (코드만 완료, tsc 0)
- [ ] [e2e] 플레이어 "비트" 토글 → 완료 곡: BPM 헤더+비트 스트립 흐름·NOW 통과 점등; seek 후 즉시 점프
- [ ] [e2e] 추출 중 곡: 3초 폴링→완료 자동 전환; 실패 곡: 내 곡만 "다시 시도"(pending 복귀), 남의 곡 미노출
- [ ] [e2e] 웹 메트로놈 ON/OFF(다운비트 낮은 음)·일시정지 즉시 정지; 네이티브는 버튼 미노출
- [ ] [e2e] 비트뷰 켠 채 곡 전환 → 리셋·재생 UI 끊김 없음(수백 비트 곡)
- [ ] [api] GET /charts/daily 응답·[e2e] 일간 탭 순위 표시, top100 전용 최신앨범 섹션이 일간 탭에 안 뜨는지(회귀)

### v3.98 — Kits 음성 변환 + MR 피치 (코드만 완료, tsc 0)
- [ ] [api] GET /kits/voice-models — 목록 로드, Kits 키 미설정 시 503 메시지 노출
- [ ] [e2e] 생성 이력 완료 카드→"내 목소리로 변환"→변환 시작→폴링 %→awaiting_merge; 화면 이탈 후 재진입 시 상태 이어짐
- [ ] [e2e] MR 패널: 보컬만/MR만/같이 재생, 볼륨 실시간, 피치 +2 프리뷰 — **native blob→캐시 wav 재생 경로 실기기 검증 필수(수십 MB)**
- [ ] [e2e] 병합→완료→병합본 재생→발매 시 use_voice_converted:true 트랙 오디오=변환본(variant 0)
- [ ] [e2e] 실패/409/타임아웃 팝업·재변환·재병합, 이미 발매된 생성의 완료 화면 안내
- [ ] 참고: ⭐ 과금 없음 실측(points 연동 0건) — confirm에 "⭐ 차감 없음" 명시됨
