...
**Q**: vocalGender = 메인, style = 서브 — 2_housing 에서 그렇게 했지?

**A**: 네, 맞아요.
- 2_housing `services/musicService.ts`:
  - `vocal` ← 메인 보컬 키만 (`'male_warm'` 등)
  - `style` ← 메인 + 서브 보컬 영문 태그 모두 (`"acoustic, soft male vocal, warm, soft female duet vocal, breathy, duet harmonized"`)
- 1_MV_wedding `services/suno_generator.py` 도 **이미 동일하게 구현되어 있음** (line 142-176):
  - `body["vocalGender"]` ← `main_info["gender"]` (메인만)
  - `style_parts` 에 main_info["style"] + sub_info["style"] + "duet, two voices alternating" 모두 추가

### 2) 진짜 갭 — 가사 프롬프트

**1_MV_wedding lyrics_generator.py (이전 코드, line 470-472)**:
```python
duet_line = ""
if vocal_form == "duet":
...
문제:
- LLM 에게 단순히 "main=female_warm, sub=male_powerful" 같은 키 이름만 전달
- LLM 이 문자열 안의 "female"/"male" 글자를 보고 [Female]/[Male] 라벨 결정
- 키 이름과 의도가 일치하면 동작하지만, **main=male_warm, sub=female_powerful 같은 역순 케이스에서 [Female] 라벨이 서브가 아닌 메인에 가는 위험** (또는 그 반대)

### 3) 구현 계획

**파일**: `1_MV_wedding/backend_8000/app/services/lyrics_generator.py`

1. **`_vocal_key_gender(vocal_key)` 헬퍼 추가** (line 363 부근):
   - vocal_styles 키 prefix 로 gender 추론
   - `female_*` → "female", `male_*` → "male", 기타 → ""
   - suno_generator.SUNO_VOCAL_MAP 와 컨벤션 공유 (의존 cycle 회피 위해 import 안 함 — 주석에 동기화 필요 명시)

2. **`_build_user_message_wedding` 의 duet_line 빌드 교체** (line ~470 부근):
   - main_gender + sub_gender 추출
   - **혼성 듀엣** (gender 다름): 프롬프트에 명시적 매핑 추가
     ```
...
3. **suno_generator.py 무수정** — 이미 2_housing 패턴과 동일

### 4) 배포 절차

```bash
# 1) 개발 PC
git add 1_MV_wedding/backend_8000/app/services/lyrics_generator.py
...
touch 1_MV_wedding/backend_8000/app/services/lyrics_generator.py  # WatchFiles 강제 트리거
```

### 5) 검증

- venv 직접 import 로 smoke test:
  - `_vocal_key_gender("female_warm")` → `'female'`
  - `_vocal_key_gender("male_powerful")` → `'male'`
  - `_vocal_key_gender(None)` → `''`
- worker PID 변경으로 reload 확인

### 6) 회귀 위험

| # | 위험 | 완화 |
|---|------|------|
| R1 | SUNO_VOCAL_MAP 키 컨벤션 변경 시 헬퍼 미동기화 | 주석으로 동기화 의무 명시 |
| R2 | 동성 듀엣 (male+male, female+female) 케이스에서 라벨 모호 | fallback (raw key 전달) — system prompt 일반 규칙으로 처리. 정식 지원은 별도 작업 |
| R3 | 이미 생성된 가사에 영향 | 없음 (다음 가사 생성부터 적용) |
| R4 | 2_housing 백엔드 (0_platform_music/backend) 수정 | 미수정 — `1_MV_wedding/backend_8000` 만 add |

### 7) 후속 (별도 작업)

- 동성 듀엣 지원 → `WEDDING_SYSTEM_PROMPT_DUET` 에 [Vocal A]/[Vocal B] 같은 중립 라벨 도입 필요
- 2_housing 에는 듀엣 가사 생성 기능 자체가 없음 (lyricsService 가 단순 prompt/genre/mood 만 받음) — 1_MV_wedding 의 wedding 전용 시점 마커 + 듀엣 라벨 규칙은 더 정교한 시스템
...
### 조사 결과
- 현재 상태: [Intro]/[Verse]/[Pre-Chorus]/[Chorus]/[Bridge]/[Outro] 등 섹션 라벨은 SOLO/DUET 두 prompt 모두에 **이미 명시**. 권장 구조 + 최소 줄수도 있음.
- **단조로움의 진짜 원인 3가지**:
  1. **2-min 작품**에서 Bridge / Verse 3 "생략 가능" 으로 명시 → LLM 이 Bridge 빼버림 → 구조 빈약
  2. **두 prompt 모두 "섹션별 음악 에너지/다이나믹스" 지시가 없음** → LLM 이 라벨만 박고 어휘 강도는 평탄 → Suno 가 평탄한 음악으로 해석
  3. `_ensure_lyrics_structure` 폴백이 라벨 없을 때 4줄마다 [Verse]/[Chorus] 기계 교차 → Pre-Chorus/Bridge/Intro/Outro 다양성 제외

### 구현 계획

**파일 1**: `1_MV_wedding/backend_8000/app/services/lyrics_generator.py`
1. SOLO prompt 룰 16 (가사 길이 가이드) 수정 — 2-min 도 Bridge 필수 (짧아도 OK), Verse 3 만 생략 가능
2. SOLO prompt 룰 17 "섹션별 에너지 아크" 신설 — 1~10 척도 + 어휘 강도 차이 예시 (낮음=구체 디테일 → 최고=단순·반복·합창), 기존 Few-shot 은 룰 18 로 이동
3. DUET prompt 룰 16 동일 수정 (Bridge 생략 금지)
4. DUET prompt 룰 17 "섹션별 에너지 아크 + 듀엣 배분 힌트" 신설 — 에너지 척도 + 단성/콜앤리스폰스/합창 배분 매핑 + [Female]/[Male]/[Both] 예시

**파일 2**: `1_MV_wedding/backend_8000/app/services/suno_generator.py`
5. `_ensure_lyrics_structure` 폴백 로직 보강 — 라벨 ≥2개: 통과 / 라벨 1개: 경고 + 통과 / 라벨 0개: emergency 폴백 (기존 4줄 교차). 부분 라벨 케이스에서 강제 보완 안 함

### 배포 계획
- 백엔드 PC selective sync + worker reload
- venv smoke test (prompt 문자열 검증 + `_ensure_lyrics_structure` 동작 검증)

### 기대 효과
- 2-min 작품도 Bridge 포함 → 구조 다양성 확보
- 섹션별 어휘 강도 차이 → Suno V5 다이나믹스 인식 → 음악 기복 생김
- LLM 이 prompt 의 에너지 아크 표를 보고 Pre-Chorus 단순화, Chorus 호명/추임, Bridge 톤 비틈
...
### 배경 / 사용자 흐름
- 2026-06-01: 사용자 "ValueError: no reference images resolved (sheet_ids=[], plac…" 에러 보고 → v32 fix (default sheet fallback)
- 2026-06-02: 사용자 "레퍼런스 없이도 영상 생성 할 수 있게 해놓으면 안되?" → v33 fix (refs 0개 시 text-only 진행)
- 두 작업 모두 **Phase 2 씬 이미지 생성 신뢰성** 이 주제 → 한 entry 로 통합 기록

### 에러 origin / 원인 chain
- 발화 지점: `pre_mv_phase2_image_generator.py:381` — `refs_count == 0` 시 `raise ValueError`
- 원인 chain:
  1. Phase 0 (시나리오 매핑) — 사용자 스토리에 `@캐릭터`/`@장소` 멘션 없으면 `scenario_events[].refs` 비움
  2. Phase 1 (씬 분할) — 그 slot 의 event 들 refs 비어있으면 scene 의 `ref_sheet_ids=[]`, `ref_place_ids=[]` 저장
  3. Phase 2 — 빈 ref 보고 ValueError, 재시도해도 DB 상태 동일 → 동일 fail
- 모델 capability 확인: `ALLOWED_IMAGE_MODELS = (gpt_image_2, nb_pro)` 둘 다 text-to-image 가능
  - `openai_image.generate_image`: ref_images 빈 리스트 → `/v1/images/generations` 자동 라우팅
  - `_call_gemini_image` (nano-banana): image_parts 빈 리스트 → text-only 모드

### 변경 파일
- 백엔드: `pre_mv_phase2_image_generator.py` (단일 파일, v32+v33 누적 +47/-5)
- 프런트엔드: 무수정

### v32 — Default sheet fallback (commit `812d2e5`, +37줄)
...
- 캐릭터 시트가 있는 사용자는 retry 시 자동 복구 → 일관성 유지

### v33 — Text-only 허용 (commit `d899a8a`, +10/-5)
`refs_count == 0` 분기를 `raise ValueError` → `logger.warning` 으로 downgrade:
- v32 fallback 까지 모두 실패 (캐릭터 시트 미생성 사용자) 해도 text-only 모드로 진행
- 캐릭터·장소 일관성은 떨어지나 ValueError 로 막히는 것보다 나음
- docstring 동기화: "ref 가 단 한 장도 없거나" → "Step A Gemini 빈 응답일 때만. ref 0개여도 진행"

### 우선순위 (실행 순서, Phase 2 ref 결정 로직)
...
- venv smoke: `raise on refs==0: False`, `warning on refs==0: True` ✅

### 기대 효과
- 캐릭터 시트 있는 사용자: 자동 ref 채워 일관성 유지 (v32)
- 캐릭터 시트 없는 사용자: text-only 로 진행 (v33). 일관성 ↓ 이지만 결과물 나옴
- 어떤 경우든 ValueError 로 fail 안 함

### 회귀 위험
| # | 위험 | 완화 |
|---|------|------|
| R1 | v33 text-only 결과물 일관성 낮아 사용자 불만 | warning 로그 모니터링 + 캐릭터 시트 생성 유도 UI |
| R2 | v32 fallback 시트가 너무 generic 한 컨텍스트에 맞지 않음 | wedding > casual 순서로 결혼식 톤 우선, 그래도 문제 시 후속 작업에서 컨텍스트 기반 시트 선택 |
| R3 | Phase 1 DB 상태는 여전히 빈 ref → 매번 Phase 2 fallback 거침 | 후속: Phase 1 측 동일 fallback 적용해 DB 에 정상 ref 박아두기 |

### 후속 (별도 작업 시)
- Phase 1 측 동일 fallback 적용 — DB 에 정상 ref 박아두기 (현재 Phase 2 fallback 으로 동일 효과)
- 씬 편집 UI 에서 사용자가 ref 직접 추가/변경
- Phase 0 시 사용자 캐릭터 시트가 있으면 자동 멘션 (UI 도움)
...
### 배경 / 사용자 흐름
- v32 default sheet fallback 적용 후 부작용: 사용자가 캐릭터 시트(예: `groom_wedding` 정장) 만 만들어 두고 ref 명시 안 했을 때, 모든 씬이 시트 옷 그대로 (정장) 으로 생성됨
- 사용자 제안: "캐릭터 시트가 인용되지 않은 slot 은 현재 캐릭터 시트의 얼굴만 사용하게 하고(옷은 상황에 맞게 변주), 캐릭터 시트가 인용된 slot 은 캐릭터 시트의 옷·얼굴 모두 사용. 시트가 없다! 하면 첫 이미지 생성부터 캐릭터시트를 디폴트로 끌어와서 작업"
...
### 변경 파일
- 백엔드: `1_MV_wedding/backend_8000/app/services/pre_mv_phase2_image_generator.py` (단일 파일, +24/-4)
- 프런트엔드: 무수정
...
4. **`_block` 헬퍼 시그니처 확장**: `face_only: bool = False` 추가 → 인물 블록 끝에 `[face-only]` or `[full-match]` 마커 자동 append
5. **groom_block / bride_block 호출**: `face_only=(not groom_is_explicit)` / `face_only=(not bride_is_explicit)` 인자 전달
6. **SCENE_IMAGE_SYSTEM_PROMPT 룰 ① 수정**:
   ...
   - 신규: 두 마커별 동작 명시
     ...
     - 예: "시트가 정장이어도 여름 비치 씬이면 가벼운 셔츠로 갈아입은 동일 인물"

### 우선순위 (실행 순서, Phase 2 ref 결정 로직, v34 누적)
...
### 기대 효과
- 같은 캐릭터 시트만 만들어 두고 ref 명시 안 해도, 씬마다 계절·장소에 맞는 옷으로 입혀짐 (face-only fallback)
- 사용자가 의도적으로 @멘션한 경우엔 시트 옷 그대로 (의도 존중)
- 기존 작품에 영향 없음. 다음 씬 이미지 생성/재시도부터 효과

### 회귀 위험
| # | 위험 | 완화 |
|---|------|------|
| R1 | LLM (Gemini Step A + 이미지 모델) 이 `[face-only]` 마커를 무시하고 시트 옷 그대로 가져옴 | 첫 1~2 씬 실측 비교. 필요 시 image_prompt 의 옷 묘사 더 명시화 (Phase 1 측 보강) |
| R2 | full-match 인데 face-only 처럼 옷 변주 발생 | system prompt 의 full-match 정의 명확 — 동일 모델이라면 face-only 도 잘 작동할 가능성 큼 |
| R3 | 마커 변경으로 기존 prompt 캐싱/로그 분석 패턴 깨짐 | 마커는 system_prompt 변경이라 작품별 로그에 영향 없음 |

### 후속 (별도 작업 시)
- 첫 1~2 씬 실측 비교 — face-only 가 실제로 옷을 변주하는지 확인
- 효과 미진하면 Phase 1 측에서 image_prompt 에 옷 묘사 명시화 보강
...
### 목표
- Phase 3 (영상) 에서 explicit @ ref slot 과 v37 fallback default 시트를 구분하여 Kling 에 다른 가이던스 전달
- explicit: 얼굴 + 옷/액세서리 모두 시트 그대로 (FULL-MATCH)
- fallback: 얼굴만 시트 그대로, 옷은 scene_prompt 따라 변주 (FACE-ONLY)
- Phase 2 image-side v34 와 동일한 의도 (이미지에서는 `[face-only]` / `[full-match]` 마커, 영상에서는 자연어 identity guidance 블록)

### Backend Part 1 (`1_MV_wedding/backend_8000/app/routes/pre_mv.py`)
1. `_load_char_sheet_bytes_for_scene` 반환 형식 변경:
   - 이전: `list[bytes]`
...
1. `generate_scene_video_kling` 시그니처 확장:
...
### v48.1 — 2026-06-06 — 장르 옵션 3개 제외
사용자 요청: "장르에서 비지엠, 피아노 발라드, 인디는 빼줘."

- 제외: `BGM`, `피아노 발라드`, `인디`
- 유지: `인디팝` (사용자가 "인디" 만 명시)
- 결과: GENRE_OPTIONS 24개 → **21개**


### v48.2 — 2026-06-06 — 장르 추가 정리
사용자 요청: "포크랑 어쿠스틱은 똑같은 말 아니야? 똑같으면 어쿠스틱으로 통일. EDM이랑 일렉트로닉도 똑같으면 EDM으로 통일. 클래식은 빼줘."

- 포크 → 어쿠스틱 흡수 (`포크` 제거)
- 일렉트로닉 → EDM 흡수 (`일렉트로닉` 제거)
- `클래식` 제거
- GENRE_OPTIONS 21개 → **18개**


### v48.2 — 2026-06-06 — 장르 추가 정리
사용자 요청: 포크/어쿠스틱 동의어 → 어쿠스틱 통합, EDM/일렉트로닉 동의어 → EDM 통합, 클래식 제외.
...
### v54.10 — Default 시트를 wedding(예복) 우선으로
- 사용자 요청대로 default 시트 선택 정책 변경.
- `_resolve_first_sheet_for_role` 정렬 키에 `style_priority` 추가:
