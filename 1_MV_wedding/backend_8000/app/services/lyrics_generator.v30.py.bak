...
duration_minutes=2 → 본문 약 600~800자 (메타태그 제외).
  필수 섹션 (v29 — 절대 생략 금지): [Intro] + [Verse 1] + [Verse 2] + [Pre-Chorus]
   + [Chorus 1] + [Bridge] + [Chorus 2] + [Outro]
  각 Verse 4~6줄, 각 Chorus 4줄, Pre-Chorus 3~4줄, Bridge 4줄, Intro/Outro 2~3줄.
  Verse 3 만 생략하거나 짧게 통합 가능. Bridge 는 짧아도 반드시 포함 (단조로움 방지).
...
# 룰 18 — Few-shot 예시 (Verse 1 톤 학습용 — 낮은 에너지 섹션)
...
(★ 위는 Verse 1 의 낮은 에너지 톤 예시일 뿐. Pre-Chorus 부터 어휘를 점진적으로 단순화·합일화하고
   Chorus 에서는 반복·호명·합창성 어휘로 최고조를 만들 것. Bridge 는 톤을 한 번 비틀어 대비를 줄 것.)
...
# 룰 17 — ★ 섹션별 에너지 아크 (★★ Suno 가 음악 다이나믹스를 인식하도록 필수)
곡 전체가 평탄하지 않으려면, 섹션마다 감정·에너지 높이가 달라야 한다.
Suno V5 는 가사 섹션 라벨 + 가사 어휘 강도 + [Female]/[Male]/[Both] 배분으로
그 섹션의 음악적 에너지를 결정한다. 가사가 평탄하면 음악도 평탄해진다.

섹션별 권장 에너지 (1~10 척도) + 듀엣 배분 힌트:
  [Intro]      낮음 (1~2) — [Both] 짧게. 분위기만 설정.
  [Verse 1]    낮음~중간 (2~4) — [Female]/[Male] 단성 교차. 한쪽 시점 위주.
  [Verse 2]    중간 (3~5) — 반대쪽 시점도 섞기. 어휘 약간 풍부.
  [Pre-Chorus] 중간~높음 (5~7) — ★ 점진적 상승. [Female]/[Male] 콜앤리스폰스 → 후반에 [Both] 합류.
  [Chorus 1]   높음 (7~9) — ★★ 최고조. [Both] 위주. 어휘는 단순·반복·합창성.
  [Verse 3]    중간 (3~5) — 감정 재설정. 다시 단성으로 내려옴.
  [Bridge]     중간~높음 (5~7) — ★ 대비/톤 전환. 한쪽([Female] 또는 [Male]) 솔로로 대비를 줘도 좋음.
  [Chorus 2]   최고 (8~10) — ★★ 최종 클라이맥스. [Both] 가 더 두드러지게. 다짐 어휘.
  [Outro]      낮음 (1~3) — [Both] 부드러운 여운.

구체 예 (어휘 강도 차이):
  Verse 1 (낮음, 단성): "[Female] 그날 카페 창가에 너의 손이 떨렸어"
  Pre-Chorus (상승, 콜앤리스폰스): "[Female] 그 순간이 / [Male] 우리를 만들었지 / [Both] 운명이었어"
  Chorus 1 (최고, 합창): "[Both] 우리 함께 걸어가, 약속한 그 길 위에서"
  Bridge (대비, 솔로): "[Male] 이제 새 페이지를 펼쳐, 두 사람의 이름으로"
  Chorus 2 (최종, 합창 강화): "[Both] 영원히 너의 곁에서, 약속을 지킬게"
...
(★ 위는 Verse 1 의 낮은 에너지 + 단성 교차 예시일 뿐. Pre-Chorus 부터 콜앤리스폰스→[Both] 합류로
   에너지 올리고, Chorus 에서는 [Both] 위주로 합창 최고조, Bridge 는 솔로로 톤을 비틀어 대비.)
