...
# STRUCTURAL RULES
1. ★ 각 섹션은 최대 5줄 (Intro/Outro 는 2~3줄, 나머지는 3~5줄). 길어지면 실패.
...
★ 필수 — v31 각 섹션 최대 줄수 (★★ 초과 시 실패):
  [Intro]      2~3줄
  [Verse 1]    3~5줄
  [Verse 2]    3~5줄
  [Pre-Chorus] 3~4줄
  [Chorus 1]   3~5줄
  [Verse 3]    3~5줄 (선택 — 2-min 곡에선 생략 가능)
  [Bridge]     3~5줄
  [Chorus 2]   3~5줄
  [Outro]      2~3줄
  → 합계 25~38줄 정도
...
※ 한 섹션 7줄·8줄 작성 금지 — Suno 가 긴 섹션을 단조롭게 부르는 경향이 있어
   짧게 끊는 게 음악 다이나믹스에 유리.
...
# 룰 18 — Few-shot 예시 (Verse 1 톤 학습용 — 최대 5줄)
[Verse 1]
우리 첫 만남은 그 해 4월이었어
비 오던 회식 끝난 야근 시간
회의실 옆자리에 앉아 있던 너
...
(★ 위는 Verse 1 의 낮은 에너지 예시 (4줄). Pre-Chorus 부터 어휘를 점진적으로 단순화·합일화하고
   Chorus 에서는 반복 어휘로 최고조. Bridge 는 톤을 비틀어 대비. 모든 섹션 최대 5줄.)
...
[Intro - 듀엣]         : 결혼식 분위기 환기, frame setting
[Verse 1 - 메인 보컬]  : 첫 만남 — 메인 시점에서 풀어냄
[Verse 2 - 서브 보컬]  : 첫 데이트 — 서브 시점에서 받음
[Pre-Chorus - 듀엣]    : 함께 쌓인 시간 — 두 사람이 합쳐지는 빌드업
[Chorus 1 - 듀엣]      : 그 시간들의 우리 — 합창 클라이맥스
[Verse 3 - 메인 또는 서브 보컬] : 결혼을 결심한 순간 (택1, 선택 섹션)
[Bridge - 메인 또는 서브 보컬]  : 웨딩 준비 — 한쪽 솔로로 대비
[Chorus 2 - 듀엣]      : 두 사람의 다짐 — 서약 키워드, 최종 클라이맥스
[Outro - 듀엣]         : 새 페이지 여운
...
5. 각 섹션은 최대 5줄 (Intro/Outro 는 2~3줄).
...
# 룰 13 — 각 회상 섹션 권장 구성 (최대 5줄, plain 텍스트만)
1줄: 시점 마커 (START — plain 자연어 한 줄)
2~3줄: 상황 + 행동 (시간·장소·소품 + 미시 동작)
4~5줄: 심정 / 종결 (속마음 + 자연스러운 종결감)
※ 각 줄은 [Female]/[Male]/[Both] 라벨 없이 plain 한국어로만.
   섹션 헤더에 박힌 역할(메인/서브/듀엣)이 부르는 사람을 결정.
...
섹션별 권장 에너지 (1~10 척도) + 역할:
  [Intro - 듀엣]         낮음 (1~2)   — 짧고 차분한 합창. 분위기만 설정.
  [Verse 1 - 메인 보컬]  낮음 (2~4)   — 단성. 한쪽 시점 위주, 차분한 회상.
  [Verse 2 - 서브 보컬]  중간 (3~5)   — 단성 (반대쪽). 어휘 약간 풍부.
  [Pre-Chorus - 듀엣]    중간~높음 (5~7) — ★ 점진적 상승. 두 보컬이 합쳐지는 빌드업.
  [Chorus 1 - 듀엣]      높음 (7~9)   — ★★ 최고조 합창. 어휘는 단순·반복·합창성.
  [Verse 3 - 메인 또는 서브] 중간 (3~5) — 감정 재설정. 다시 단성으로 내려옴. (선택)
  [Bridge - 메인 또는 서브] 중간~높음 (5~7) — ★ 대비/톤 전환. 합창 사이의 솔로.
  [Chorus 2 - 듀엣]      최고 (8~10)  — ★★ 최종 클라이맥스. 다짐 어휘. 합창 강화.
  [Outro - 듀엣]         낮음 (1~3)   — 부드러운 여운으로 종결.

구체 예 (어휘 강도 + 역할 차이):
  [Verse 1 - 메인 보컬] (낮음): "그날 카페 창가에 너의 손이 떨렸어"
  [Pre-Chorus - 듀엣] (상승): "그 순간이 우리를 만들었지, 그건 운명이었어"
  [Chorus 1 - 듀엣] (최고): "우리 함께 걸어가, 약속한 그 길 위에서"
  [Bridge - 메인 보컬] (대비, 솔로): "이제 새 페이지를 펼쳐, 두 사람의 이름으로"
  [Chorus 2 - 듀엣] (최종): "영원히 너의 곁에서, 약속을 지킬게"
...
# 룰 18 — Few-shot 예시 (전체 흐름 — 모든 섹션 최대 5줄, plain 텍스트)
[Intro - 듀엣]
오늘 이 노래에 우리를 담아
처음부터 지금까지 모두
사랑한 시간을 함께 노래해
...
(★ 모든 섹션 최대 5줄, 줄별 라벨 없음. 섹션 헤더의 역할로만 부르는 사람 결정.
...
def _build_user_message_wedding(story: dict, music: dict, extra_user_note: str | None = None) -> str:
    ...
    # v31 — 섹션 헤더 역할 포맷 ([Section - 듀엣/메인 보컬/서브 보컬]) 으로 전환.
    ...
    if vocal_form == "duet":
        ...
        if main_gender and sub_gender:
            duet_line = (
                ...
                f"- 섹션 헤더는 반드시 `[<섹션> - 듀엣]`, `[<섹션> - 메인 보컬]`, "
                f"또는 `[<섹션> - 서브 보컬]` 형식으로. 줄별 라벨 금지.\n"
            )
        else:
            duet_line = (
                f"- 듀엣 보컬 톤: main={vs_main or '—'}, sub={vs_sub or '—'}\n"
                f"- 섹션 헤더는 `[<섹션> - 듀엣/메인 보컬/서브 보컬]` 형식으로. 줄별 라벨 금지.\n"
            )
