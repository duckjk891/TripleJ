...
export class MapRenderer {
  ...
  render(): void {
    ...
    // 3. 그리드 라인 (디버그용, 옵션)
    // this.renderGrid(worldWidth, worldHeight, tileSize);

    // 4. 영역 이름 표시
    AREAS.forEach(area => this.renderAreaLabel(area));
  }

  private renderBackground(worldWidth: number, worldHeight: number): void {
    const graphics = this.scene.add.graphics();
    graphics.fillStyle(0x5a8f3e, 1);  // 잔디색
    graphics.fillRect(0, 0, worldWidth, worldHeight);
    graphics.setDepth(0);  // 배경은 가장 아래
  }
  ...
  private renderAreaLabel(area: AreaDefinition): void {
    ...
    const { tileSize } = MAP_CONFIG;
    const x = area.x * tileSize + 5;
    const y = area.y * tileSize + 5;

    const text = this.scene.add.text(x, y, area.name, {
      fontSize: area.type === 'area' ? '14px' : '11px',
      color: '#ffffff',
  ...
  private renderGrid(worldWidth: number, worldHeight: number, tileSize: number): void {
    const graphics = this.scene.add.graphics();
