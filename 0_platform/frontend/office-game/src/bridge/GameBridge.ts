...
export class GameBridge implements IGameBridge {
  ...
  public sendCommand<T extends GameCommandType>(command: GameCommand<T>): void {
    ...
    if (!this.isGameReady) {
      ...
      console.log('[GameBridge] Command queued:', command.type);
      return;
    }
  ...
  private processCommand(command: GameCommand): void {
    console.log('[GameBridge] Processing command:', command.type, command.payload);
  ...
  private postMessageToParent(message: unknown): void {
    try {
      ...
      // 크로스 오리진 에러 무시
      console.debug('[GameBridge] postMessage error (cross-origin):', error);
    }
  }
  ...
  public disconnectScene(): void {
    ...
    console.log('[GameBridge] Scene disconnected');
  }
  ...
  public connectWebSocket(): void {
    ...
    this.gameWebSocket.setCallbacks({
      ...
      onConnectionStateChange: (state: WSConnectionState) => {
        ...
        console.log('[GameBridge] Backend connection:', state);
      },
    });
  ...
  public destroy(): void {
    ...
    console.log('[GameBridge] Destroyed');
  }
   ...
   */
  public debug(): void {
    console.log('[GameBridge] Debug Info:');
    console.log('  - isReady:', this.isGameReady);
