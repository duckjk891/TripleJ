...
async def generate_character_sheet(
    ...
    step_a_prompt = (
        ...
        # v241(대표 지적 2026-09-11): 미화 방지 — 외모 묘사는 참조 사진의 실제 특징 그대로
        "외모 묘사 규칙(필수): [인물 사진]이 있으면 얼굴 생김새·이목구비 비율·얼굴형·"
        "머리 모양/길이/색·체형·키 느낌·피부 톤과 피부 특징(점·주근깨·잡티 등)을 "
        "사진에 보이는 그대로 구체적으로 기술하라. 미화·이상화·보정(피부 매끈화, "
        "얼굴 축소, 눈 확대, 체형 슬림화 등)을 유도하는 표현을 넣지 말 것 — "
        "'beautiful/perfect/flawless' 류의 수식 대신 관찰된 사실만 적는다.\n\n"
        + master
    )
    ...
    if has_photo:
        step_b_prompt = (
            ...
            # v241(대표 지적): 미화 금지 — 실제 특징 충실 재현
            "FAITHFUL LIKENESS(필수): 참조 사진의 실제 특징을 그대로 재현하라 — "
            "얼굴 생김새와 이목구비 비율, 얼굴형, 머리 모양·길이·색, 체형, 피부 톤, "
            "피부 특징(점·주근깨 등)을 미화·이상화·보정 없이 유지할 것. "
            "피부를 매끈하게 다듬거나, 눈을 키우거나, 얼굴/체형을 날씬하게 바꾸는 등 "
            "'더 예쁘게' 만드는 변형을 금지한다. 닮음이 아름다움보다 우선이다.\n"
