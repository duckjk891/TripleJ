...
async def apply_mv_watermark(input_path: str, output_path: str) -> bool:
    """v137 [watermark]: MV 최종본 가시 워터마크 1 pass (재인코딩 1회).

    drawtext "MAIDOL · AI 생성" — 1920x1080 기준 fontsize 40 우하단 +
    컨테이너 메타데이터 comment="AI-Generated by MAIDOL". 오디오는 copy.
    Returns True on success, False on failure (호출부에서 원본 유지 폴백).
    """
    ...
    proc = await asyncio.create_subprocess_exec(
        ffmpeg_bin, "-y",
        "-i", input_path,
        "-vf", build_drawtext(40, x="w-tw-40", y="h-th-40"),
    ...
    if proc.returncode != 0:
        logger.warning(
            "[watermark] mv drawtext pass failed rc=%s: %s",
            proc.returncode, stderr.decode()[:500],
        )
        return False
    logger.info("[watermark] mv drawtext applied output=%s", output_path)
    return True
...
async def concatenate_videos(video_paths: List[str], output_path: str) -> None:
    ...
    v137 [watermark]: concat 후 최종본에 가시 워터마크 1 pass 추가
    (실패 시 무워터마크 concat 결과로 폴백 — MV 산출 자체는 깨지 않음).
    """
    ffmpeg_bin = _get_ffmpeg_path()
    if not ffmpeg_bin:
        raise RuntimeError(
    ...
    # v137: concat 결과는 중간 파일로 받고, 워터마크 pass 를 거쳐 output_path 산출.
    concat_out_path = output_path + ".concat.mp4"
    try:
        ...
        # v137 [watermark]: 최종본 가시 워터마크 pass.
        if not await apply_mv_watermark(concat_out_path, output_path):
            logger.warning(
                "[watermark] mv pass skipped — concat 원본으로 폴백 output=%s",
                output_path,
            )
            os.replace(concat_out_path, output_path)
    finally:
        # Clean up concat list + intermediate
...
async def run_mv_pipeline(
    ...
    try:
        ...
        # v137 [watermark]: 단일 씬(concat 스킵) 경로도 최종본 워터마크 보장.
        # (concat 경로는 concatenate_videos 내부에서 이미 적용됨.)
        if len(ordered_video_paths) == 1 and _ffmpeg_available():
            _wm_path = os.path.join(tmpdir, "final_wm.mp4")
            if await apply_mv_watermark(final_video_path, _wm_path):
                final_video_path = _wm_path
