...
async def verify(
    ...
    if singer_skill_level < 1 or singer_skill_level > 5:
        raise HTTPException(status_code=400, detail="singer_skill_level 은 1~5 사이여야 합니다.")

    verify_object = await _save_audio_to_minio(user_id, clone_id, "verify", verify_file)
