...
### 1. 요청 작업
v196 후속 논의에서 나온 커버 이미지 관련 2건. 사용자 지시: "둘 다 확인해줘" → "진행해줘".
**착수 시 2건이었으나 0단계 감사에서 동급 결함 2건이 추가로 발견되어 4건 + 프론트 1건으로 확대**됐다.

### 2. 결함 목록 (전부 선재 결함, 9006 전환과 무관)

| # | 결함 | 실측 근거 |
|---|---|---|
| ① | **직접 첨부한 커버가 100% 저장 실패** | `UploadPage.jsx:1566` 이 `type='track'` 전송 / `upload.py:121` 은 `("cover","profile")` 만 수용 → **400**. 실호출로 재현. `.catch(() => {})` 가 에러를 삼켜 화면에는 "업로드가 완료되었습니다!" 만 표시 → **사용자·개발자 모두 실패를 인지 못 함**. `frontend/src` 에서 `/upload/image` 호출은 **`UploadPage.jsx` 단 1곳**, `type=cover` 분기는 **도달 불가 코드**였다 |
| ② | **커버 변경에 소유권 검사 없음** | `upload.py:154` `update_one({"_id": ObjectId(id)}, ...)` — `upload.py` 922줄 전문에 `uploader_id` **0건**. 대조군 `tracks.py:715`·`:738` 은 `403 "자신의 트랙만…"` 으로 정상 차단 |
| ③ | 🆕 **`/upload/mv-status` 도 동일 계열** | `upload.py:848~858` 이 `mv_jobs.find_one({"_id":...})` 만 하고 `user_id` 대조 없음. 같은 컬렉션을 쓰는 `mv.py:202~208` 에는 `_get_job_with_ownership`(400/404/403) 이 있는데 **`upload.py` 만 빠짐**. 로그인만 하면 타인의 미공개 MV 진행률 + 씬 썸네일 + **최종 영상 presigned URL** 획득 = 유료 생성물 사전 유출 |
| ④ | 🆕 **커버 변경 후 캐시 미무효화** | `GET /api/tracks/{id}` 가 `cache:track:v3:{id}` 를 **600초** 캐싱(`tracks.py:1183`,`:1298`). `update_track`·`purge`·`admin` 은 전부 지우는데 `upload.py` 만 안 지움 → ①을 고쳐도 최대 10분간 옛 커버 표시 |

**기우로 판명된 것**: `refine-cover`·`revert-cover`·`cover-history` 는 `_load_cover_session`(`:481`)이 `user_id` 를 대조하고 있어 **이미 정상**. `generate-cover`·`generate-mv` 는 트랙 미참조. `type=profile` 분기는 입력 `id` 를 쓰지 않고 `current_user['id']` 로만 경로를 만들어 **안전**. → 결함은 `/image(cover)` 와 `/mv-status` **2곳으로 확정**.

**경로 함의(②의 심각도)**: 커버 키가 `covers/{요청자 id}/{곡 id}` 라서 남의 곡을 바꾸면 ① 피해 트랙의 `cover_image_url` 이 **공격자 폴더 키**로 바뀌어 경로 기반 소유권 추론이 오염되고 ② `purge_track_document` 는 **현재 `cover_image_url` 하나만** 지우므로 **피해자 원본이 영구 고아**가 되며 피해자가 곡을 지워도 삭제되는 건 공격자 객체다 ③ 이전 값 보존·알림이 없어 복구 불가.

### 3. 핵심 설계 결정

| 항목 | 결정 | 근거 |
|---|---|---|
| ① `type` | **서버가 `track`→`cover` 폐기예정 별칭으로 수용 + 프론트는 `cover` 로 정정 (둘 다)** | 앱팀 클라이언트가 보내는 값을 **확인할 수 없음**(앱 코드가 이 PC 에 없음) → 서버는 깨지 않고 수용. 동시에 웹을 정정해야 **"정정 후에도 남는 `track` = 앱팀"** 이라는 계측 신호가 오염되지 않아 별칭 제거 시점을 판단할 수 있다. 옵션 ①만 하면 앱이 고장난 채 남고, ②만 하면 틀린 값이 웹에 영구 고착 |
| ② 403 문구 | `"자신의 트랙만 수정할 수 있습니다."` | `tracks.py:738 update_track` 과 자구 동일(커버 변경은 삭제가 아니라 수정) |
| ② 없는 id | **404** | 트랙 계열은 일관되게 404→403 2단계이고 트랙 id 는 차트·URL 로 이미 공개되어 은닉 실익 없음 |
| ② 가드 위치 | 🔴 **`put_object` 이전** | 뒤에 두면 무단 객체 생성이 그대로 남아 수정의 절반이 무의미. `generate_cover` 의 포인트 선차감 前 게이트 관행과 동일 |
| ④ 캐시 삭제 | **try/except 필수** | 감싸지 않으면 Redis 장애가 **기존에 성공하던 커버 업로드를 500 으로 바꾼다**. 최악은 최대 600초 표시 지연 |

### 4. 수행 결과 (백엔드 1파일 + 프론트 1파일)
- **`upload.py`** (+58/−4) — A1 순수함수 `_normalize_upload_image_type` 신설(`_normalize_image_model`·`_normalize_vocal_gender` 관행 승계) / A2 진입부 교체 + `type_invalid`(앱팀 실제 값 회수용)·`type_alias_deprecated` 로그, **400 본문·상태코드 불변** / A3 소유권 가드 404·403 을 **`put_object` 이전**에, `find_one` 1회 + 프로젝션 / A4 캐시 무효화 try-except + `cover_ok ... matched=%d` / A5 `profile_legacy_route` 계측 로그(동작 무변경) / A6 `mv_status` 403(`mv.py` import 없이 `JSONResponse` 관행 유지)
- **`UploadPage.jsx`** (+7/−2) — `type` `'track'`→`'cover'`, `.catch(()=>{})` → `try/catch` + `console.error`(DEV 가드 없음 — 프로덕션 흔적 필요) + `setError('곡은 업로드되었지만 커버 이미지 저장에 실패했습니다…')` + `return`(거짓 성공·자동이동 차단). **바깥 `finally { setUploading(false) }` 무수정** — `try` 내부 `return` 도 `finally` 를 실행하므로 버튼이 잠기지 않음(코드 분석 + 런타임 재현 2중 확인)

### 5. 테스트 결과 — **20건 + 회귀 8건 전부 PASS, 결함 0건**
`[unit]` 8(40%) / `[api]` 7(35%) / `[e2e]` 5(25%). 즉시중단 조건 S1~S8 **전부 미발생**.

**반쪽 수정 감시(이번 설계의 핵심)** — 403 만 확인하면 가드가 `put_object` **뒤**에 있어도 통과한다. 2중으로 감시했고 둘 다 통과:
- `UNIT-07`: 404·403 분기에서 `put_object` 스텁 호출 **0회**
- `API-03`: 403 후 MinIO `covers/<A>/` 목록이 **착수 전과 완전히 동일** = `covers/<A>/<TRK-B>.jpg` **미생성**

그 외 주요 실측: `type=track` 별칭 201 + 로그 1건 / `type=banner` 400 본문 자구 v197 이전과 동일 / 미존재 id 404(403 아님) / `A4` — 캐시 적재 → 업로드 → **두 키 모두 0** → 즉시 재조회에서 새 커버(600초 대기 없이) / `A6` — 타인 403 시 응답에 `result_video_url`·`scene_thumbnails` 키 **전부 부재** / `E2E-01` 목록 썸네일 `naturalWidth/Height 64×64` = **첨부 원본 크기**(placeholder 아님) / `E2E-03` 에러 배너 노출 + 성공 메시지 미노출 + 자동이동 없음 + **버튼 재활성** / `E2E-05` 프로필은 `/auth/me/profile-image` 로 나가고 **512×512 크롭 적용**, `/upload/image` 0건

**회귀 8건**: AI 커버 경로 무영향(`tracks.py` diff **0줄**) / 프로필 업로드 무영향(profile 분기 diff = **로그 1줄 추가뿐**) / 커버 미첨부 업로드 정상 / `type=profile` 여전히 201(**4중 검증** — A2 의 `norm_type` 교체 누락 검출용) / `refine`·`revert`·`history` diff 0건 / `TRK-NOOWNER`(uploader_id 키 부재) → 403 이 정상, 실 DB 해당 문서 **0건** / `mv.py` diff **0줄** / `frontend_admin` diff **0줄**

**유료 호출**: `POST /api/tracks/upload` **정확히 1회**(상한 2회, 예비 미사용, 약 0.2원). 금지 경로 전건 0회 — `tracks/search`·`/api/generate`·`/api/mv/`·`character`·`voice-*`·`generate-cover`·`refine-cover`·`cs/broadcast` 각 0. 별 차감 0.
**테스트 데이터**: `tracks` 23→23, `mv_jobs` 67→67, MinIO `covers/<A>`·`<B>` 0·0, PG `users.profile_image`(A) NULL→**NULL 원복**, `git status` untracked 0.

### 6. 기록 사항 (결함 아님)
- **A1 델타 사후 승인**: 구현이 PLAN 설계에 없던 `matched=%d` 를 `cover_ok` 로그에 추가 — 반영 건수를 담은 개선이므로 정상 처리
- **E2E-01 의 `type` 파트 직접 판독 불가**: Playwright 가 File 첨부 multipart 의 `postData()` 를 반환하지 않음 → 2중 우회 증명(E2E-03 인터셉트가 동일 경로에서 `type=cover` 판독 + E2E-01 서버 로그에 `cover_ok` 는 있고 `type_alias_deprecated` 는 **0건**)
- **TESTPLAN 문서 오기 3건**: 커버 버킷명, 상세 라우트(`/player?track=`), 기존 catch 로그 문구 오기 — 전부 구현 무관
- **`run.sh` 의 `tee` 가 재기동마다 `server.log` 절단** → 재기동 전 로그 보존이 필요하면 `tee -a` 검토 건의

### 7. 범위 밖 / 후속 과제
- 🔴 **`mv-preview` 에 경로 탈출(`..`) 가드 부재** — 형제인 `cover-preview` 는 v173 에서 획득했으나 `mv-preview` 만 비대칭. **별도 보안 항목으로 승계 권고**
- `/upload/image` 의 `type=profile` 분기 vs 정식 `/auth/me/profile-image` 이원화 — 정식 경로에만 있는 3가지(512×512 크롭 / 이전 이미지 정리 / Redis 세션 갱신). **앱팀이 아직 legacy 경로를 쓴다면 크롭 없는 프로필 + stale 세션을 겪는다.** A5 계측 로그로 사용량을 측정한 뒤 통합 판단
- 확장자 변경 시 고아 커버 객체 잔존 / 커버 변경 이력·알림 부재
- `cache:track:v2:` 는 `admin.py:862` 에서만 삭제되고 **쓰는 곳이 전무** — 읽기 경로는 `v3` 뿐(정리 후보)
- ⚠️ **v196 §7 미해결 항목 유효**: 앱팀이 사내망으로 쓰는 `backend_9004` 에는 v196·v197 수정이 **전부 미반영**(같은 실데이터 참조). 사용자 판단으로 앱팀의 9006 이전 시점까지 유보

### 8. 변경 파일 (커밋 대상 5)
- 백엔드 1: `backend_9006/app/routes/upload.py`
...
## v201-r — 2026-08-24 — v201(차트 캐시) 사후 재검토 + limit 클램프 [MAIDOL-ChartCacheReview]

> 사용자 지시: v201 은 대기 지시를 오독한 planner 의 선진행이었음 → **team-dev 로 사후 재검토**. 착수 HEAD `03264b2`.

### 1. 재검토 결과 — 사용자 판단이 옳았다. v201 에 구멍이 있었다

v201 본체(캐시 키에 limit 포함)는 올바르나, **무검증 limit 이 키에 들어가면서 새 위험 표면이 생겼다** (planner 0단계 실측):
- `?limit=999999999` → 통과 + `cache:chart:top100:999999999` 키 생성 → **limit 루프로 Redis 키 무한 생성 가능**
- `?limit=-5` → 파이썬 음수 슬라이스로 1곡 반환(오동작) + `:-5` 키 생성

### 2. 보완 — `charts.py:304` **+6줄(주석 5 + 코드 1)**
`limit = max(1, min(limit, 100))` — 화이트리스트 검증(:294) 뒤, cache_key 조립(:313) **앞**.
422 거부가 아닌 클램프인 이유: 기존 호출자 무영향 + 앱팀 클라이언트가 임의 값을 보내도 안 깨짐. 키 공간이 5종×100=**최대 500개로 유한**해지고 음수 슬라이스도 함께 차단.

### 3. 테스트 — 13건 중 **12 PASS / 1 FAIL(판정 기준 오류로 최종 PASS 처리)**
`[unit]` 6(46.2%) / `[api]` 7(53.8%) / e2e 0(허용). 즉시중단 S1~S4 미발생, 픽스 루프 0회.

핵심 실측: 경계값 9종 `[-5,0,1,2,99,100,101,200,999999999]` → `[1,1,1,2,99,100,100,100,100]` 완전 일치 / `:-5`·`:0`·`:999999999` 키 **미생성** / `limit=200` 과 `100` 이 **같은 키 1개** 공유(키 증가 0, hit-vs-hit **93,726바이트 완전 동일**) / v201 본체 `:3`·`:100` 병존(선점 오염 없음) / 무효화 3곳 전부 `cache:chart:*` 패턴 + 정확 키 delete 잔재 0 / `top1000` → 400 + 키 미생성 / genre·category 재기동 전후 diff 0 / 유료 0 · 9004/9005 접근 0 · DEL 은 `cache:chart:*` 범위 내 4건뿐.

**A03 "miss vs hit 바이트 동일" FAIL 의 planner 판정 — 클램프 결함 아님, 판정 기준 오류.**
원인: miss 응답은 FastAPI 가 datetime 을 `2026-06-10T08:40:55`(ISO) 로, 캐시 저장은 `json.dumps(result, default=str)`(`charts.py:337`, **커밋 `03264b2` 에 이미 존재** — git show 재확인) 가 `2026-06-10 08:40:55`(공백) 로 직렬화 → 구조상 바이트 동일 불가. 클램프의 판정 목적(같은 실효값=같은 키·캐시 공유)은 다른 두 단언으로 충족됨.

### 4. 범위 밖 기록 (선재, 미수정)
- **miss/hit 직렬화 불일치**: 캐시 히트 응답의 datetime 류(`beats_completed_at` 등)가 miss 와 포맷이 다르다(`T` vs 공백). 프런트 사용처 **0건**(실측)이라 실영향 없음. 근본 수정은 캐시 저장 전 `jsonable_encoder` 적용 — 후속 후보
- 검증 중 잔존 `cache:chart:*` 6키는 TTL(300s) 자연 소멸 궤도 확인
...
## v207 — 기존 곡 커버 이미지 수정 기능 + update_track 커버 검증 신설 [MAIDOL-CoverEditSquad] (2026-08-26 19:22)

> 사용자 요구: 업로드한 내 곡(공개·비공개 불문)의 커버를 수정 단계에서 교체 — 파일 첨부 / 내 캐릭터 AI / 프롬프트 AI (업로드 때와 동일 방식·동일가 ⭐5). 진입점 2곳(목업 승인). 착수 HEAD `050b2e1`.

### 1. 설계 요점
- **새 API 0개** — 기존 3종 조합으로 완결: `generate-cover`(⭐5 차감 내장·곡 비종속) + `/upload/image(type=cover)`(v197 소유권 가드) + `update_track(cover_image_url)`
- **진입점 2곳**: 「내 음악」›「내 트랙」 곡 카드(공유·삭제 사이) / 「내 채널」›「곡·앨범」 SongItem — **옵션 prop 방식**이라 차트·검색 등 공용 사용처 6곳 렌더 무변화
- **공용 모달 `CoverEditModal`**: 현재→새 커버 듀얼 미리보기, 탭 3, 다시 생성(재차감 confirm), 되돌리기. 교체 성공 시 모달 유지+성공 바(완료·되돌리기) — "즉시 닫으면 되돌리기 도달 불가" 모순을 dev 가 발견해 해소

### 2. 🔴 보안 — 기능이자 패치였다
0단계 가설("update_track 이 커버 값을 무검증 저장")이 **실측으로 확정** (`tracks.py:724` — v197 소유권 가드 뒤에서 임의 문자열 $set). 방치 시 타인 오브젝트·faces/(얼굴인증)·외부 URL 을 커버로 지정 가능.
→ `_validate_cover_image_url` 신설. 허용 = **서버가 결정적으로 증명 가능한 집합만**: ① 본인 cover_sessions 산출물(현재본+refine 이력) ② 본인 파일커버 결정적 경로 `covers/{uid}/{tid}.ext` ③ 기존값 동일(revert — 레거시 http 의 유일 통로). 차단 = faces/·evidence/(동일값이어도)·외부 URL·타인 세션·`..`·임의 문자열·빈 문자열. None 은 완전 무통과(기존 사용처 무영향 — 호출처 0곳 실측). 캐시 무효화는 기존재(`:758~762`) 확인만.

### 3. 테스트 — 16/16 PASS (unit 7 · api 6 · e2e 3), 즉시중단 0, 픽스 루프 0
- **보안 실서버 재현**: 타계정 공격 2종 403 / faces·타인세션·외부URL 400(본문 `유효하지 않은 커버 이미지입니다.`) + `[cover-edit] rejected` warning / 허용 3종 src=session·file·revert info 로그 — 검증 함수 단위 14/14 와 이중 확인
- **캐시 즉시성**: 캐시 프라임 → 교체 → **즉시 재조회에서 새 값** (600s 대기 없음)
- **E2E 9단계**(파일 첨부 전 과정, Windows Chrome 경유): 로그인→내 트랙→모달→첨부→교체→성공 바→즉시 갱신→새로고침 유지. 내 채널 본인 버튼 노출 / **타인 채널·차트·검색·비로그인 버튼 0** / AI 탭은 ⭐5 표기·열람만(실호출 0)
- **데이터 위생**: 일회용 계정·v207test 마커 문서·1×1 오브젝트 3중 원복 — leftover 0(이전 실패 런 고아 2계정까지 정리). generate-cover 호출 0·⭐ 차감 0·실사용자 무접촉

### 4. 관찰 기록 (FAIL 아님 — 후속 후보)
- **커버 오브젝트 수명 문제(v197 §7 승계 확장)**: ① 한 세션 산출물을 두 곡이 공유 가능 → 한 곡 purge 시 다른 곡 커버 소실 가능 ② 커버 교체 시 구 오브젝트 고아 잔존 ③ cover_sessions 는 purge 미대상. 커버 오브젝트 참조 관리(복사-소유 or 참조 카운트)를 별건으로 승계
- 「내 트랙」 카드에는 커버 썸네일 img 가 원래 없음 — 목록 갱신 판정은 모달·내 채널 썸네일로 어댑트
- 되돌리기는 세션 이력이 아닌 "모달 열 때 원값" 방식 — revert 통로로 서버 검증 통과 확인
- **선재 버그 발견(칩 발행)**: UploadPage 의 기존 refine 호출이 문자열 payload 라 `refine_prompt` 미전송 → 422 추정. 신규 모달은 올바른 형태. 칩 「업로드 화면 커버 재수정(refine) 422 버그 수정」
...
## v209 — 「내 음악」 작업 탭 재편: 작사실·작곡실·MV촬영실 분리 (2026-08-26 22:10)

### 1. 요청 요지 (그림 합의 확정 사양)
「내 음악」 탭 재편: ① 「작업실2」→**「작사실」** 개명·작사 전용화 — AI 작사(⭐) 또는 직접 작성 → 저장, 작업 영역 바로 아래 내 작사 리스트(보기·수정·삭제) ② **「작곡실」** 신설 — 작사 리스트에서 1개 땡겨오기 → 파라미터 → 곡 생성(⭐), 완료 리스트에 가사 연결 표시 + 곡별 [업로드→](새 업로드로 프리필 이동) ③ **「MV촬영실」** 신설 — 곡 풀 = 작곡실 미업로드 완성곡 + 내 트랙 공개 + 비공개 통합 목록, 곡 선택 시 가사·커버 자동 땡김 → 씬→이미지→영상→합치기 ④ 구 「작업실」 유지 ⑤ UploadPage 내장 MV 흐름은 MV촬영실로 이전 후 제거.

### 2. 설계 결정 (PLAN v209 §1·§7~§12)
- **D1** 작사/작곡 분할선 = StudioTab2 `handleConfirmLyrics`(:927-962). 작사실=step1-2+직접작성, 작곡실=step3-4+생성기록+피로게이지+Wondera+기존 onSendToUpload 재사용
- **D2** 내 작사 저장소 = 기존 draft(generations, start_music_gen:false 무과금) 재활용 + `PATCH /generate/{id}` 신설(신규 컬렉션·삭제후재생성 기각). 작사 ⭐5 유실 창은 생성 즉시 자동 영속으로 봉합
- **D3** MV 곡 소스 갭 = mv create `track_id` 확장 + 소유권 검증(프론트 역추적 기각 — 파일 업로드 트랙은 generation 부재)
- **D4** 커버 필수 400 → MV촬영실 커버 확인 스텝 사전 가드(작곡실 완성곡은 전원 커버 부재 실측)
- **D5** MV = 이동 우선: MVProductionSection 기계적 추출(결합 6지점 — 실측 5 + coverImageModel 복원 1 신규 발견 — 콜백/ref 배선) 후 MVStudioTab이 래핑
- **D6** 임시저장 탭 유지 + 불러오기 타겟만 MV촬영실 리타겟(탭 제거는 ▲사용자 확인 대기)
- **D7** 탭 구성: 내 트랙/내 앨범/새 업로드/작업실(무변경)/작사실('studio2' 키 유지)/작곡실('compose')/MV촬영실('mvstudio')/내 캐릭터/임시저장
- **D8** 파일 업로드 트랙 lyrics 저장 누락 — lyrics 1필드만 이번 포함(tracks.py +3), cover/mv object name 파라미터는 별건 칩. 기존 저장분 소급 불가 → 가사 없는 곡 허용 UX 편성
- **D9** 응답값 확정: 타인 track/draft 403, 무효 형식 400·미존재 404, PATCH 가드 400→404→403→409
- 추가 판단: mv create "소스 둘 다 부재" 400 가드 **미도입**(임시저장 create가 소스 없이 호출되는 정상 경로 실측 — 도입 시 즉시 파손, §9) / PATCH·create `duet` 대칭 저장(11→12필드+create doc 2줄, §10-11) / persistDraft prompt 폴백 승인(직접 작성 저장 사양 충족, §10)

### 3. 구현 이력 (단계별)
| 단계 | 담당 | 내용 |
|---|---|---|
| 0 | backend-dev | mv.py: CreateMVRequest.track_id + 검증(무효400/미존재404/타인·blinded 403/무오디오400) + audio_track_id 저장·duration 보정, mv_pipeline 3단 폴백(tracks.audio_url). generate.py: PATCH /generate/{id}(가드 4종+화이트리스트 12필드·duet create 대칭 2줄). tracks.py: /tracks/upload doc에 lyrics 저장. api/index.js: updateGeneration 신설·createMVJob track_id 명시 통과(하위호환) |
| 1 | frontend-dev | UploadPage 4348→(중간)·MVProductionSection 신설 기계적 추출, 결합 6지점 ref/콜백 배선, 모달 type="button" 5개, vite build 통과 — UploadPage가 계속 렌더(동작 불변) |
| 2 | frontend-dev | studioShared(353)·LyricsStudioTab(905)·ComposeStudioTab(2194) 분할, 탭 배선(작사실 라벨/compose 신설), onSendToCompose(draftDoc)→composePrefill 인계, 작사 즉시 영속·리스트 CRUD(PATCH) |
| 3 | frontend-dev | MVStudioTab(489) 신설 — 곡 풀 3소스 통합·커버 확인 스텝·MV 초안 리스트·draftData 복원, UploadPage MV 제거(→1119줄)·StudioTab2 삭제, 임시저장 리타겟('mvstudio'), 편차 2건 해소(querySelector 제거→오디오 props 계약·prefix [MVStudio] 통일), duet 복원, 가사 없는 곡 허용 UX, 커버 object name 직전달 |
| 픽스 루프 1회 | backend+frontend | T14 발견 track_id 유실 — SaveDraftRequest+`_validate_user_track_source` 헬퍼 추출·GET 상세 audio_track_id 노출(서버) / 저장 body·복원 관통·getTrackDetail 재조회·blinded 필터(프론트) → 재검 PASS |

### 4. 테스트 판정 — 전체 PASS (18건 + 재검, TESTPLAN v209 :10159~)
| 게이트 | 시나리오 | 결과 |
|---|---|---|
| 단계0 | T04(모델 단위)·T05(PATCH 가드 5종+화이트리스트 주입 방어)·T06(track_id 가드 4종, 직삽 포함)·T07(draft 라이프사이클·무과금 실증) | PASS |
| 단계1+2 | T01(추출 대조)·T03(불변 파일)·T08(작사실 CRUD·⭐5 표기)·T09(작사 즉시 영속 — 인터셉트 fulfill→자동 draft)·T10(작곡 인계·⭐15·2계열 인터셉트)·T11(완료 리스트·[업로드→] 프리필)·T15/T16(v207 커버·v208 refine·제출 A/B 스모크)·T18a | PASS (편차 2건 → 3단계 편입) |
| 단계3+4 | T02(절단 최종 — querySelector 0건·prefix 통일 포함)·T12(곡 풀 3소스+가사 없는 트랙 ⑤)·T13(가사·커버 자동 인계·커버 스텝 가드·/mv/create body)·T14(MV 이관 동작 보존 — 픽스 1회 후 재검)·T17(임시저장 리타겟)·T18·T15/T16 재실행·save-draft 서버 검증 4종+성공 왕복(직삽 실호출) | PASS |
- 안전 증빙: ⭐ 계정 A~E 전 구간 차감 0, 유료 엔드포인트 서버 도달 0(계획된 4xx 실호출 제외), 계정·mongo 직삽·임시파일 전량 원복(탈퇴 cascade 없음 실측 — 명시 DELETE 원칙), 9004/9005 무접촉, 9006 정상

### 5. 인시던트·관찰·이월
- **인시던트 1**(단계2 T10 1차): translate-tags 인터셉트 누락 → 실서버 경유 OpenAI 실호출 3건(⭐ 무관, 미세 외부 과금). 즉시 인터셉트 추가로 차단, 이후 0건. TESTPLAN §0에 translate-tags 명문화 + 부수 외부 호출 전수 grep으로 차단 목록 폐쇄
- 관찰: generation 소스의 audio_duration_sec가 요청값 그대로 저장(서버 재검증 없음) — 별건 후보
- 이월 별건: ① MV 오디오 소스 부재 merge 단계 사전 가드+좀비 job 정리(칩 등록됨) ② 파일 업로드 시 AI 커버·MV object name 파라미터 드롭(칩 등록됨) ③ 곡 풀 페이지네이션(현행 1페이지 로드) ④ MV촬영실 커버 확인 스텝의 캐릭터·장소 옵션 미지원(파일 첨부·기본 생성만) ⑤ uploadFromGeneration is_public=True 하드코딩(비공개 발행 요구 시)
- ▲ 사용자 확인 대기: 임시저장 탭 제거 여부(현행 유지+리타겟) / 라벨 'MV 임시저장' 변경 여부
- 해소됨: duet create 대칭(§11), querySelector 지뢰, prefix 혼재, T14 track_id 유실

### 6. 안전 준수
유료 성공 경로 실행 0(인시던트 1건 즉시 차단·기록) · 일회용 테스트 계정 가입→탈퇴 원복 · 실사용자 데이터·실 .env 무접촉 · 인프라 무조작(9006 재기동만 PID kill→setsid ./run.sh) · 9004/9005 접근 0 · 산출물 시크릿·실이메일 0

### 7. 변경 파일 (커밋 대상 16)
신설 5: `frontend/src/components/MVProductionSection.jsx`(3279) · `frontend/src/components/studio/MVStudioTab.jsx`(489) · `frontend/src/components/studio/LyricsStudioTab.jsx`(905) · `frontend/src/components/studio/ComposeStudioTab.jsx`(2194) · `frontend/src/components/studio/studioShared.js`(353)
수정 7: `frontend/src/pages/UploadPage.jsx`(→1119) · `frontend/src/pages/MyMusicPage.jsx` · `frontend/src/api/index.js`(+6, 전량 v209 확인) · `backend_9006/app/routes/generate.py`(+104) · `backend_9006/app/routes/mv.py`(±87) · `backend_9006/app/routes/tracks.py`(+3) · `backend_9006/app/services/mv_pipeline.py`(+24)
삭제 1: `frontend/src/components/StudioTab2.jsx`(-3078)
산출물 3: `claude_skills_outputs/team-dev/{PLAN,TESTPLAN,REPORT}.md`

## v210 — 이월 별건 2건: 업로드 산출물 저장 누락 + MV 소스 가드·좀비 job 정리 (2026-08-27 01:29)

### 1. 배경 (v209 이월 칩 2건)
① 파일 업로드 시 AI 커버·MV 산출물(object name) 저장 누락 ② MV 오디오 소스 부재 가드 + 좀비 job 정리. v209 탭 재편(작사실/작곡실/MV촬영실, UploadPage MV 제거) 완료 후 **현재 코드 기준 전면 재실측**으로 착수.

### 2. 실측 발견 (PLAN v210 §0)
- **커버 유실 실버그**: UploadPage 분기 B는 지금도 `cover_object_name`을 FormData로 전송(:425)하나 `/tracks/upload`(tracks.py :1394-1409)에 수신 Form 파라미터가 없어 서버 드롭 — "AI 커버(⭐5) 생성 → 파일 업로드 제출" 시 커버가 조용히 유실되는 현행 버그(배선 복구 불요, 서버 수신만 필요).
- **분기 A 무검증**: upload-from-generation은 `cover_object_name`을 검증 없이 `cover_image_url`에 저장(:1736 구) — v207이 update_track에서 막은 구멍(임의 문자열→faces/·타인 오브젝트)과 동일 계열이 생성 경로에 잔존.
- **mv_object_name**: 프론트 전송 0건(v209 제거 확정) + 완성 MV→트랙 연결 경로 현존 0(onMvObjectNameChange 소비자 0, tracks 23건 중 mv 필드 0) → **서버 파라미터 미신설**, body의 데드 필드는 유지+주석.
- **소스 없는 mv create 호출부 전수(2곳뿐)**: ①handleCreateScenes는 양 소스 전송(단 draft 복원 곡은 이론상 둘 다 null 가능) ②**saveDraftInternal이 create body에 audio_generation_id 누락**(track_id만 실음 — generation 소스 곡 임시저장마다 소스 없는 create+파이프라인(:662 무조건 발동) 후 saveMVDraft 사후 부착; 실패 시 sourceless job 잔존 경로). merge 단계는 pydantic 필수+프론트 3중 가드로 기가드.
- **좀비 실측**(mongo·MinIO read-only): mv_jobs **67건**(images_ready 33/failed 13/videos_ready 8/completed 6/paused 5/video_ready 2), 전건 updated_at ≤ 2026-05-25(90일+ stale, 미오픈 dev 데이터), sourceless 0건. MinIO `music-platform-images/mv/` **8.5GiB·1,413 objects**(v198 8.66GB 정합) — completed 3.78GiB/images_ready 2.11GiB/video_ready 1.48GiB/videos_ready 0.88GiB/paused 0.11GiB/failed 0(문서만)+thumbnails 14MiB·generated 82MiB. 고아 prefix 0, 로컬 잔해 0. owner용 `DELETE /mv/jobs/{id}` 기존재.

### 3. 설계 결정 (PLAN v210 §1·§7)
- **D1** /tracks/upload에 cover_object_name Form 신설 + 검증 후 저장. 검증 = v207 규칙의 생성 시점 재사용판 헬퍼 `_validate_cover_object_name_for_create`(**session 소유 분기만** — 생성 시점엔 track doc 부재로 revert/file 분기 불성립. faces/·evidence/·`..`·http 차단). 실패 400(⭐5 지불 커버의 silent drop 기각), **MinIO put 이전 검증**.
- **D2** 분기 A에 동일 헬퍼 적용(무검증 저장 봉합) — 정상 흐름(세션 산출물)은 전량 통과라 하위호환 무손상, 미전송 None은 기존대로.
- **D3** mv_object_name 파라미터 미신설(죽은 코드 방지) — "완성 MV→트랙 연결"은 별도 설계 이월(v209 B0-3 병합).
- **D4 가드 택1 = (a) create 400** (`audio_generation_id·track_id` 둘 다 부재 시, cover 400 직후·mongo 접근 전) + 프론트 2건(saveDraftInternal create body에 audio_generation_id — 그 자체 버그 픽스 / handleCreateScenes 사전 가드 disabled+alert). 근거: DB sourceless 0건(파손 실데이터 없음)·잔존 호출부 1건은 픽스 대상·(b)는 draft 잠김 job의 무한 0% 폴링으로 더 나쁨·(c)는 merge 기가드+create 방치 시 phase1/2 외부 API 낭비 지속.
- **D5** 좀비 정리 = **read-only 목록화 스크립트까지만**(`scripts/mv_cleanup_report.py` — 후보 기준: failed·paused 또는 90일+ stale·최종 산출물 부재. 삭제 코드 0줄). 실삭제·관리자 API 미도입(전건 dev 데이터+기존 DELETE API 존재 — 과대 설계 방지).

### 4. 구현 (픽스 루프 0회 — 경미 편차 1건은 DEV 로그 1줄로 종결)
| 담당 | 내용 |
|---|---|
| backend-dev | tracks.py +91/-1(헬퍼 :604-639·/upload 수신+검증 :1469-1489·분기 A :1678-1694·데드 필드 주석 :1603-1606), mv.py +10(소스 가드 :414-417), scripts/mv_cleanup_report.py 178줄 신설(read-only) |
| frontend-dev | MVProductionSection.jsx +16/-1 — F1 create body audio_generation_id(:1132), F2 이중 가드(버튼 disabled+핸들러 :388), DEV 로그 `[MVStudio] create body {has_gen,has_track}`(:1124). F3 분기 B 배선 무수정 확인. vite build 통과 |
| planner | dev 검수 PASS + TESTPLAN 조정 3건(M3 실구현 문구·판정 서열 정정 / U계열 purge cascade 주의 / R계열 충분 판정) + Q1-Q4 전건 선답(§7) |

### 5. 테스트 판정 — **12/12 PASS** (TESTPLAN v210 :10528~, [api]5/[unit]2/[e2e]5, 게이트 2단)
| 축 | 시나리오 | 결과 |
|---|---|---|
| 커버 봉합 | U1(세션 두 분기 통과·저장) · U2(거부 4종 400 + 트랙/MinIO 미생성 계측 = put 이전 증명) · U3(미전송 None 불변) · U4(분기 A 통과/400/미전송) · R1(분기 A/B 제출 3종 배선 + mv_object_name 부재) | PASS |
| mv 소스 가드 3면 | M1(서버 400·mv_jobs count 불변·phase 로그 0 = 파이프라인 미발동 계측) · M2(create body audio_generation_id 캡처) · M3(버튼 disabled + 네트워크 0건) | PASS |
...
- 별건(보안 기록): merge-audio `audio_object_name` 임의 값 무검증 + create `audio_generation_id` 소유권 무검증.
- 유지: v209 이월분 중 곡 풀 페이지네이션·커버 스텝 캐릭터/장소 옵션·is_public 하드코딩.

### 7. 안전 준수
유료 성공 경로 실행 0(mv create 2xx 금지 준수·400 케이스만 실호출) · 일회용 테스트 계정 가입→탈퇴 · 실사용자 데이터·실 .env 무접촉(mongo/MinIO 조회 read-only, 자격증명은 컨테이너 내부 env 참조 — 산출물 시크릿 0) · 인프라 무조작(9006 재기동만 PID kill→setsid ./run.sh) · 9004/9005 접근 0 · 산출물 시크릿·실이메일 0
