...
(async()=>{
  ...
  let k1Payload=false;
  if(coverPayload){const p=JSON.parse(coverPayload);k1Payload=p.shot==='전신'&&p.palette==='다크 무디'&&p.background_prompt==='보라색 스튜디오'&&!p.lyrics_excerpt;log('K1 payload:',JSON.stringify(p).slice(0,260));}
  ...
  await clickT('이대로 만들기 (건너뛰기)');await sleep(4000);
  let k2Payload=false;
  if(coverPayload){const p=JSON.parse(coverPayload);k2Payload=!!p.lyrics_excerpt&&!p.shot&&!p.palette;log('K2 payload lyrics len:',(p.lyrics_excerpt||'').length,'shot:',p.shot||null);}
