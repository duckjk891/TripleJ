...
import { listVoiceClones, VoiceClone } from '../services/voiceService';

// ── 내 목소리(Voice Clone) 스토어 ───────────────────────────────────────────
// 목록은 항상 서버에서 재조회(영속화 안 함).
// v3.84: "아티스트 목소리"는 프리셋(성별+보컬 스타일 태그) 또는 클론 중
// 하나만 배타적으로 가진다(artistVoice). 프리셋은 서버 자산이 아니라 곡 생성 태그로만 쓰임.
// v3.102: 구 Voice Persona 목록(personas/fetchPersonas) 제거 — v216 서버 엔드포인트 삭제,
// 내 목소리는 voice_clones 체계로 일원화.

/** 아티스트 목소리 — 프리셋(간편)과 클론(내 목소리)은 서로 배타.
 *  clone.personaId 에는 Suno voice_id(clone.voice_id)가 들어간다 — 작곡 persona_id 전송값(v216 권장 경로). */
