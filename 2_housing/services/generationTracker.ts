...
function processingJobs(): TrackedJob[] {
  return listUserArtistJobs().filter((j) => j.lastStatus === 'processing');
}
...
async function tick(): Promise<void> {
  ...
  try {
    for (const job of targets) {
      if (_hidden) break;
      // startedAt+30분: 서버 lazy stale 정리(failed+환불 1회)를 recoverable로 유도 → 아래 GET이 결과를 따른다
      if (Date.now() - job.startedAt >= STALE_MS && !job.staleProbeAt) {
      ...
      try {
        ...
        console.warn('[GenTracker] 조회 오류 — 상태 유지·백오프', {
          jobId: job.jobId, status: err?.response?.status ?? null, message: err?.message,
        });
      }
    }
  } finally {
...
async function verifyDoneJobs(): Promise<void> {
  const done = listUserArtistJobs().filter((j) => j.lastStatus === 'done');
  for (const j of done) {
    try {
...
function maybeNotifyDone(): void {
  ...
  const job = listUserArtistJobs().find(
    (j) => j.lastStatus === 'done' && !j.notifiedAt && j.jobId !== _viewerJobId
  );
  if (!job) return;
