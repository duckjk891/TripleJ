...
// step: 0=장르, 1=분위기, 2=스타일, 3=주제, 4=키워드, 5=시점, 6=언어, 7=구조, 8=길이, 9=랩, 10=추가요청
...
export default function LyricsInputScreen({ navigation }: Props) {
  ...
  const processAnswer = (answer: string, currentStep: number) => {
    // 0=장르, 1=분위기, 2=스타일, 3=주제, 4=키워드, 5=시점, 6=언어, 7=구조, 8=길이, 9=랩, 10=추가요청
    switch (currentStep) {
    ...
    if (nextStep >= STEPS.length) {
      ...
      setTimeout(() => {
        ...
        const prompt = `${store.language} ${store.genre} 장르의 ${store.mood} 분위기 노래 가사를 작성해주세요. 곡 길이는 약 ${durationText}이고, ${store.perspective} 시점으로 ${store.content}에 대한 내용입니다. 구조는 ${store.structure}${rapText}.${kwText}${refText}`;
  ...
  return (
    ...
    >
      ...
      {chatHistory.some((m) => m.type === 'user') && (
        <View style={{ paddingTop: insets.top + 8, paddingHorizontal: 16, paddingBottom: 4, backgroundColor: '#0a0a1a' }}>
          <Text style={{ fontSize: 11, color: '#666', textAlign: 'center' }}>이전 답변을 탭하면 다시 선택할 수 있어요</Text>
        </View>
      )}
